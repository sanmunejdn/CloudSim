#ifndef GEOMETRYSERVICES_GEOMETRYREF_H
#define GEOMETRYSERVICES_GEOMETRYREF_H
...
GEOMETRY_SERVICES_EXPORT bool discretizeFeatureList(const geoalgo::FeatureListDocument& doc,
													const geoalgo::ShapeHandle& shape, geoalgo::RawPath& out,
													std::string* errMsg = nullptr);
...
GEOMETRY_SERVICES_EXPORT bool validateFeatureListDocument(const geoalgo::FeatureListDocument& doc,
														  std::string* errMsg = nullptr);

GEOMETRY_SERVICES_EXPORT bool enumerateFeatureCatalog(const geoalgo::WorkpieceRef& workpiece,
													  geoalgo::FeatureCatalog& out, std::string* errMsg = nullptr);

GEOMETRY_SERVICES_EXPORT bool enumerateFeatureCatalog(const geoalgo::WorkpieceRef& workpiece,
													  const geoalgo::ShapeHandle& shape, geoalgo::FeatureCatalog& out,
													  std::string* errMsg = nullptr);

GEOMETRY_SERVICES_EXPORT std::string featureCatalogToJson(const geoalgo::FeatureCatalog& catalog);

GEOMETRY_SERVICES_EXPORT bool suggestFeaturesFromCatalog(const geoalgo::FeatureCatalog& catalog,
														 const std::string& intentUtf8,
														 geoalgo::FeatureListDocument& out,
														 std::string* errMsg = nullptr);

GEOMETRY_SERVICES_EXPORT bool computeFeatureAnchor(const geoalgo::WorkpieceRef& workpiece,
...
buildFeatureEntryFromModelPick(const geoalgo::WorkpieceRef& workpiece, const geoalgo::ShapeHandle& shape,
							   const std::string& strategyId, bool pickFace, const geoalgo::Point3d& modelPointA,
							   const geoalgo::Point3d& modelPointB, geoalgo::FeatureEntry& out,
							   std::string* errMsg = nullptr, int knownFaceIndex = -1, int knownEdgeIndex = -1);

} // namespace geometry_backend_ops

#endif // GEOMETRYSERVICES_GEOMETRYREF_H

    ...
    std::string* errMsg)
{
    ...
    try
    {
        // 测试1: 小点云法线估计 (1万点)
        {
            ...
            result.executionTimeMs = measureExecutionTime([&]() {
        ...
        // 测试4: 小点云离群移除 (1万点)
        {
            ...
            result.executionTimeMs = measureExecutionTime([&]() {
                removeOutliers(xyz, 5.0, 24, nullptr, nullptr, nullptr);
            });
        ...
        // 测试6: 小点云Poisson重建 (1万点)
        {
            ...
            std::string err;
            result.executionTimeMs = measureExecutionTime([&]() {
                reconstructPoisson(xyz, normals, soup, 0.0, 20.0, 30.0, 0.375, &err);
            });
        ...
        // 测试8: 配置版本API (自动下采样)
        {
            ...
            std::string err;
            result.executionTimeMs = measureExecutionTime([&]() {
                reconstructPoissonAutoWithConfig(xyz, soup, config, &err);
            });
            result.success = !soup.empty();
            if (!result.success)
            {

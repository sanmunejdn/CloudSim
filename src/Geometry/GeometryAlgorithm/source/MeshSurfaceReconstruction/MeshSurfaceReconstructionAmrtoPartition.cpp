...
bool partitionQuadDomainsAmrtoImGmcg(
	...
	std::string* errMsg,
	QuadMeshLite* outCachedQuadMesh,
	GmcgResult* outCachedGmcg)
{
	patches.clear();
	outJunctionCount = 0;
	...
	{
		const std::string root = params.amrtoGoldenDataPath.empty()
			? defaultAmrtoGoldenDataDirectory()
			: params.amrtoGoldenDataPath;
		const std::string resultName = params.amrtoGoldenResultObj.empty()
			? "smooth_060.obj"
			: params.amrtoGoldenResultObj;
	...
	else
	{
		...
		{
			const std::string goldenRoot = params.amrtoGoldenDataPath.empty()
				? defaultAmrtoGoldenDataDirectory()
				: params.amrtoGoldenDataPath;
			const std::string resultName = params.amrtoGoldenResultObj.empty()
				? "smooth_060.obj"
			...
			if (params.amrtoFallbackGoldenOnImFailure && fs::is_directory(goldenQuadDir))
			{
				...
				RunLogger::warn(
					std::string("instant meshes unavailable; falling back to golden partition loader: ")
					+ goldenRoot);
		...
		{
			const fs::path workDir = params.gmcgWorkDir.empty()
				? (fs::temp_directory_path() / "CloudSim_gmcg_work")
			...
			exeParams.globalResultObjName = params.amrtoGoldenResultObj.empty()
				? "result_gmcg.obj"
				: params.amrtoGoldenResultObj;

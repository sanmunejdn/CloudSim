...
bool projectRayOntoTriangleSoup(
	...
	bool& outHit)
{
	...
	for (size_t t = 0; t < triCount; ++t)
	{
		...
		const double v0[3] = {
			triangleSoupWorldMm[base + 0],
			triangleSoupWorldMm[base + 1],
			triangleSoupWorldMm[base + 2] };
		const double v1[3] = {
			triangleSoupWorldMm[base + 3],
			triangleSoupWorldMm[base + 4],
			triangleSoupWorldMm[base + 5] };
		const double v2[3] = {
			triangleSoupWorldMm[base + 6],
			triangleSoupWorldMm[base + 7],

...
GEOMETRY_ALGORITHM_API void computeTriangleSoupNormals(
	const std::vector<float>& soup,
	std::vector<float>& outNormals);

/// 固定 Medium 精度离散（导入/首帧显示，不依赖视口）
GEOMETRY_ALGORITHM_API bool tessellateShapeMedium(
...
GEOMETRY_ALGORITHM_API bool tessellateShapePerFaceMedium(
	const ShapeHandle& shape,
	std::vector<float>& outSoup,
...
GEOMETRY_ALGORITHM_API bool tessellateShapeForView(
	const ShapeHandle& shape,

/// @file StoreZipExtract.cpp
/// @brief STORE zip 解压（自 Widget 约定移植，不链 Widget）
...
bool isStoreZipArchive(const QString& filePath)
{
	QFile f(filePath);
...
bool packStoreZipArchive(const QString& zipFilePath, const QString& rootDir, QString* errorMessage)
{
	QDir root(rootDir);
	if (!root.exists())
	{
...
bool extractStoreZipArchive(const QString& zipFilePath, const QString& destDir, QString* errorMessage)
{
	QDir dest(destDir);

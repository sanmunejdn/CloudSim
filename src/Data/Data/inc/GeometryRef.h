...
DATA_EXPORT bool resolveGeometryRef(const GeometryRef& ref, geoalgo::WorkpieceRef& out, std::string* errMsg = nullptr);

DATA_EXPORT WorkpieceShapeSource resolveWorkpieceShape(const std::string& backendIdUtf8, BackendDataManager& mgr,
													   const std::string& stepPathUtf8Optional,
													   geoalgo::ShapeHandle& outShape, geoalgo::WorkpieceRef& outRef,
													   std::string* errMsg = nullptr);

DATA_EXPORT bool discretizeFeatureList(const geoalgo::FeatureListDocument& doc, geoalgo::RawPath& out,
									   std::string* errMsg = nullptr);

DATA_EXPORT bool discretizeFeatureList(const geoalgo::FeatureListDocument& doc, const geoalgo::ShapeHandle& shape,
									   geoalgo::RawPath& out, std::string* errMsg = nullptr);

DATA_EXPORT bool featureListFromJson(const std::string& jsonUtf8, geoalgo::FeatureListDocument& out,
									 std::string* errMsg = nullptr);
...
DATA_EXPORT bool enumerateFeatureCatalog(const geoalgo::WorkpieceRef& workpiece, geoalgo::FeatureCatalog& out,
										 std::string* errMsg = nullptr);

DATA_EXPORT bool enumerateFeatureCatalog(const geoalgo::WorkpieceRef& workpiece, const geoalgo::ShapeHandle& shape,
...
DATA_EXPORT bool suggestFeaturesFromCatalog(const geoalgo::FeatureCatalog& catalog, const std::string& intentUtf8,
											geoalgo::FeatureListDocument& out, std::string* errMsg = nullptr);

DATA_EXPORT bool computeFeatureAnchor(const geoalgo::WorkpieceRef& workpiece, const geoalgo::FeatureGeometry& geometry,
									  geoalgo::FeatureAnchor& out, std::string* errMsg = nullptr);

DATA_EXPORT bool computeFeatureAnchor(const geoalgo::WorkpieceRef& workpiece, const geoalgo::ShapeHandle& shape,
									  const geoalgo::FeatureGeometry& geometry, geoalgo::FeatureAnchor& out,
									  std::string* errMsg = nullptr);

DATA_EXPORT void ensureFeatureDiscretizersRegistered();

DATA_EXPORT bool ensureFeatureDiscretizerConfigsLoaded(const std::string& resourceBaseDir,
													   std::string* errMsg = nullptr);

DATA_EXPORT std::vector<std::string> featureDiscretizerListStrategyIds();

DATA_EXPORT std::vector<geoalgo::FeatureDiscretizerParamField>
...
DATA_EXPORT bool buildFeatureEntryFromModelPick(const geoalgo::WorkpieceRef& workpiece,

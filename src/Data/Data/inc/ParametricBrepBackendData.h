...
public:
	...
	/// rebuild 后：某特征执行前/后的 tip（阵列贡献体 = after CUT before）
	geoalgo::ShapeHandle tipBeforeFeature(const std::string& featureId) const;
...
private:
	...
	std::unordered_map<std::string, geoalgo::ShapeHandle> m_tipBeforeFeature;

...
DATA_EXPORT bool sampleTriangleSoupToPointBuffers(const std::vector<float>& triangleSoup,
...
DATA_EXPORT bool buildPointCloudFromMeshForTemplateBrep(const MeshBackendData& mesh, PointCloudBackendData& outScan,
...
DATA_EXPORT bool updateBrepFromAlignedScan(const BrepBackendData& templateBrep, const PointCloudBackendData& scanCloud,
...
/// Vcg 修复 + 法矢光顺（曲面重构预处理）
DATA_EXPORT bool preprocessMeshSoupForSurfaceReconstruct(const std::vector<float>& soup,
														 const geoalgo::MeshSurfaceReconstructParams& params,
														 std::vector<float>& outSoup,
														 geoalgo::MeshSurfaceReconstructReport& report,
														 std::string* errMsg = nullptr);

DATA_EXPORT geoalgo::MeshSurfaceReconstructSessionPtr
createMeshSurfaceReconstructSession(std::vector<float> preprocessedSoup);

DATA_EXPORT bool runMeshSurfaceReconstructStage(geoalgo::MeshSurfaceReconstructSession& session,
...
DATA_EXPORT bool buildPartitionColoredMeshSoup(const geoalgo::MeshSurfaceReconstructSession& session,
											   std::vector<float>& outSoup, std::vector<float>& outRgbPerVertex,
											   std::string* errMsg = nullptr);

DATA_EXPORT bool buildSamplePointsCloud(const geoalgo::MeshSurfaceReconstructSession& session,
										std::vector<float>& outXyz, std::vector<float>& outRgba,
										std::string* errMsg = nullptr);

DATA_EXPORT bool buildFitPreviewShape(const geoalgo::MeshSurfaceReconstructSession& session,
									  geoalgo::ShapeHandle& outShape, std::string* errMsg = nullptr);

DATA_EXPORT bool meshSurfaceReconstructShapeToBrep(const geoalgo::ShapeHandle& shape,
												   std::shared_ptr<BrepBackendData>& outBrep,
												   std::string* errMsg = nullptr);

/// 网格 soup → B 样条 B-rep 曲面重构（含可选 vcg 预处理）
DATA_EXPORT bool reconstructBrepFromMeshSoup(const std::vector<float>& soup,
											 const geoalgo::MeshSurfaceReconstructParams& params,
											 std::shared_ptr<BrepBackendData>& outBrep,
											 geoalgo::MeshSurfaceReconstructReport& report,
											 std::string* errMsg = nullptr);

DATA_EXPORT geoalgo::TubularGrindingSessionPtr createTubularGrindingSession(std::vector<float> sourceSoup);

DATA_EXPORT geoalgo::TubularGrindingSessionPtr createTubularGrindingSessionFromPointCloud(std::vector<float> pointXyz);

DATA_EXPORT bool runTubularGrindingStage(geoalgo::TubularGrindingSession& session, geoalgo::TubularGrindingStage stage,
										 const geoalgo::TubularGrindingParams& params,
										 geoalgo::TubularGrindingReport& report, std::string* errMsg = nullptr);

DATA_EXPORT bool buildTubularGrindingSegmentColoredMeshSoup(const geoalgo::TubularGrindingSession& session,
															std::vector<float>& outSoup,
															std::vector<float>& outRgbPerVertex,
															std::string* errMsg = nullptr);

DATA_EXPORT bool buildTubularGrindingFpfhRegionColoredMeshSoup(const geoalgo::TubularGrindingSession& session,
															   std::vector<float>& outSoup,
															   std::vector<float>& outRgbPerVertex,
															   std::string* errMsg = nullptr);

DATA_EXPORT bool buildTubularGrindingRingColoredMeshSoup(const geoalgo::TubularGrindingSession& session,
														 std::vector<float>& outSoup,
														 std::vector<float>& outRgbPerVertex,
														 std::string* errMsg = nullptr);

DATA_EXPORT bool buildTubularGrindingRingCenterPointsCloud(const geoalgo::TubularGrindingSession& session,
														   std::vector<float>& outXyz, std::vector<float>& outRgba,
														   std::string* errMsg = nullptr);

DATA_EXPORT bool buildTubularGrindingFaceNormalAxisLineSegments(const geoalgo::TubularGrindingSession& session,
																const geoalgo::TubularGrindingParams& params,
																std::vector<float>& outLineXyz,
																std::string* errMsg = nullptr);

/// Phase 1 局部轴线线段（双向可视化）
DATA_EXPORT bool buildTubularGrindingLocalAxisLineSegments(const geoalgo::TubularGrindingSession& session,
														   const geoalgo::TubularGrindingParams& params,
														   std::vector<float>& outLineXyz,
														   std::string* errMsg = nullptr);

/// 椭圆拟合残差报告（每环 RMS + 全局摘要）
DATA_EXPORT bool computeTubularGrindingEllipseResidualReport(const geoalgo::TubularGrindingSession& session,
															 const geoalgo::TubularGrindingParams& params,
															 std::vector<double>& outPerRingRmsResiduals,
															 std::string& outSummaryText,
															 std::string* errMsg = nullptr);

DATA_EXPORT bool buildTubularGrindingCenterlinePointsCloud(const geoalgo::TubularGrindingSession& session,
														   std::vector<float>& outXyz, std::vector<float>& outRgba,
														   std::string* errMsg = nullptr);

DATA_EXPORT bool buildTubularGrindingCenterlinePolylineXyz(const geoalgo::TubularGrindingSession& session,
														   std::vector<float>& outXyz, std::string* errMsg = nullptr);

DATA_EXPORT bool buildTubularGrindingCenterlinePcaAxisArrowLineSegments(const geoalgo::TubularGrindingSession& session,
																		std::vector<float>& outLineXyz,
																		std::string* errMsg = nullptr);

DATA_EXPORT bool buildTubularGrindingTemplatePointsCloud(const geoalgo::TubularGrindingSession& session,
														 std::vector<float>& outXyz, std::vector<float>& outRgba,
														 std::string* errMsg = nullptr);

DATA_EXPORT bool buildTubularGrindingProjectedPointsCloud(const geoalgo::TubularGrindingSession& session,
														  std::vector<float>& outXyz, std::vector<float>& outRgba,
														  std::string* errMsg = nullptr);

DATA_EXPORT int tubularGrindingIterationSnapshotCount(const geoalgo::TubularGrindingSession& session);

DATA_EXPORT int tubularGrindingIterationSnapshotIteration(const geoalgo::TubularGrindingSession& session,
														  int snapshotIndex);

DATA_EXPORT bool buildTubularGrindingIterationSnapshotPointsCloud(const geoalgo::TubularGrindingSession& session,

...
static bool meshParseObjInt(const std::string& s, int& out)
{
	if (s.empty())
	{
		return false;
...
static bool meshParseObjFaceToken(const std::string& token, MeshObjCorner& corner)
{
	...
	{
		return !token.empty() && meshParseObjInt(token, corner.v);
	}
	if (!meshParseObjInt(token.substr(0, slash1), corner.v))
	{
		return false;
	}
	...
	if (slash2 == std::string::npos)
	{
		...
		if (!vtPart.empty() && !meshParseObjInt(vtPart, corner.vt))
		{
			return false;
		}
		return true;
	...
	if (!vnPart.empty() && !meshParseObjInt(vnPart, corner.vn))
	{
		return false;
	}
	return true;
...
bool meshTryLoadObjWithVertexNormals(const std::string& path, std::vector<float>& soup, std::vector<float>& normalSoup)
{
	try
	{
		...
		auto emitTriangle = [&](const MeshObjCorner& c0, const MeshObjCorner& c1, const MeshObjCorner& c2)
		{
			...
			// 退化三角不入 soup，法线也跳过，保持逐三角对齐
			if (!meshPushTri(soup, p0->x, p0->y, p0->z, p1->x, p1->y, p1->z, p2->x, p2->y, p2->z))
			{
				return;
			}
	...
	{
		// 损坏 OBJ 不得崩出 loadFromFile，交给调用方回退 CGAL
		soup.clear();
		normalSoup.clear();
		return false;
	}

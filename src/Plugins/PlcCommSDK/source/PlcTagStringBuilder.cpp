/// @file PlcTagStringBuilder.cpp
/// @brief libplctag Modbus name：co/di/hr/ir + 从 0 起的寄存器号
...
{
	return lower.size() >= 3 && (lower.rfind("co", 0) == 0 || lower.rfind("di", 0) == 0 || lower.rfind("hr", 0) == 0 ||
								 lower.rfind("ir", 0) == 0);
}
								...
								std::string* outAttributeString, std::string* outError)
{
	...
	{
		oss << "protocol=ab_eip"
			<< "&gateway=" << urlEncodeMinimal(connection.gateway) << "&path=" << urlEncodeMinimal(connection.path)
	...
	else
	{
		...
		oss << "protocol=modbus_tcp"
			<< "&gateway=" << urlEncodeMinimal(connection.gateway) << "&port=" << port
			<< "&path=" << modbusUnitIdPath(connection) << "&name=" << urlEncodeMinimal(modbusName);
		if (tag.elemCount > 0)
		{
			oss << "&elem_count=" << tag.elemCount;
		}

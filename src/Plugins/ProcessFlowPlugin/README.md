...
- 支持：**批量凑批**、**装配汇合**、**MTBF/MTTR**、**检测报废**、缓冲/仓库容量阻塞、**班次日历**、**固定/指数到达**
- **对比全部策略**：指标表；勾选「对比含甘特」可在对话框按策略切换甘特
- **优化后仿真**：`PriorityListScheduler` 推荐策略后再跑 DES
- 节点 **AGV**（车队容量 + 运输时间）；工位可填 binding（Preview 执行器可记录）

详见 `docs/ARCHIVE_ZIP_LOCATION.txt`、`docs/ARCHIVE_ZIP_LOCATION.txt`、`docs/ARCHIVE_ZIP_LOCATION.txt`。
...
| `docs/ARCHIVE_ZIP_LOCATION.txt` | UI / 模块设计 |
| `docs/ARCHIVE_ZIP_LOCATION.txt` | DES 引擎设计 |
| `docs/ARCHIVE_ZIP_LOCATION.txt` | 分阶段验收 |
| `docs/ARCHIVE_ZIP_LOCATION.txt` | 后续待办 |
| [`CloudSimPluginSDK/DEVELOPER_GUIDE.md`](../CloudSimPluginSDK/DEVELOPER_GUIDE.md) | 插件 SDK |
| [全库文档入口](../../README.md) · [产品索引](../README.md) | 宿主插件架构 |

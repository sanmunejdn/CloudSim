...
SketchArc::SketchArc() : SketchEntity() {}

SketchArc::SketchArc(const PointID& centerPointId, double radius, double startAngle, double endAngle)
	: SketchEntity(), m_centerPointId(centerPointId), m_radius(std::max(0.0, radius)),
	  m_startAngle(normalizeAngle(startAngle)), m_endAngle(normalizeAngle(endAngle))
{
	if (radius < 0.0)
	{
...
{
	return gp_Pnt2d(centerPos.X() + m_radius * std::cos(m_endAngle), centerPos.Y() + m_radius * std::sin(m_endAngle));
}

gp_Pnt2d SketchArc::midpoint(const gp_Pnt2d& centerPos) const
{
	double midAngle = m_startAngle + sweepAngle() * 0.5;
	return gp_Pnt2d(centerPos.X() + m_radius * std::cos(midAngle), centerPos.Y() + m_radius * std::sin(midAngle));
}
...
{
	return gp_Pnt2d(centerPos.X() + m_radius * std::cos(angle), centerPos.Y() + m_radius * std::sin(angle));
}
...
{
	if (!json.contains("center") || !json.contains("radius") || !json.contains("startAngle") ||
		!json.contains("endAngle"))
	{
		return false;
	}

	if (!json["center"].isString() || !json["radius"].isDouble() || !json["startAngle"].isDouble() ||
		!json["endAngle"].isDouble())
	{
		return false;
	}
	...
	EntityID newId = json.contains("id") ? json["id"].toString().toStdString() : generateId();
	bool newConstruction = json.contains("construction") ? json["construction"].toBool() : m_isConstruction;
	bool newReferenceLocked = json.contains("referenceLocked") ? json["referenceLocked"].toBool() : m_isReferenceLocked;
	...
	qCDebug(logSketchArc) << "deserialize:done"
						  << "id=" << QString::fromStdString(m_id) << "referenceLocked=" << m_isReferenceLocked
						  << "center=" << QString::fromStdString(m_centerPointId) << "radius=" << m_radius;
	return true;
}
...
BoundingBox2d SketchArc::boundsWithCenter(const gp_Pnt2d& centerPos) const
{
	...
	const double cardinalAngles[] = {0.0, std::numbers::pi_v<double> * 0.5, std::numbers::pi_v<double>,
...
bool SketchArc::isNearWithCenter(const gp_Pnt2d& testPoint, const gp_Pnt2d& centerPos, double tolerance) const
{
...
void SketchArc::dragEndpoint(const gp_Pnt2d& centerPos, bool isDraggingStart, const gp_Pnt2d& newPos)
{

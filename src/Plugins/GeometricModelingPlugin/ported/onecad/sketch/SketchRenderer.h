 ...
 */
#ifndef GEOMETRICMODELINGPLUGIN_SKETCHRENDERER_H
#define GEOMETRICMODELINGPLUGIN_SKETCHRENDERER_H
...
struct Viewport
{
	...
	Vec2d getMin() const { return {center.x - size.x / 2, center.y - size.y / 2}; }

	Vec2d getMax() const { return {center.x + size.x / 2, center.y + size.y / 2}; }
	...
	bool intersects(const Vec2d& boundsMin, const Vec2d& boundsMax) const
	{
		Vec2d vpMin = getMin();
		Vec2d vpMax = getMax();
		return !(boundsMax.x < vpMin.x || boundsMin.x > vpMax.x || boundsMax.y < vpMin.y || boundsMin.y > vpMax.y);
	}
};
...
public:
     ...
     */
	void setPreviewArc(const Vec2d& center, double radius, double startAngle, double endAngle);
     ...
     */
	void setPreviewEllipse(const Vec2d& center, double majorRadius, double minorRadius, double rotation);
     ...
     */
	void showSnapIndicator(const Vec2d& pos, SnapType type, const Vec2d& guideOrigin = {0.0, 0.0},
						   bool hasGuide = false, const std::string& hintText = "");
...
private:
     ...
     */
	std::vector<Vec2d> tessellateArc(const Vec2d& center, double radius, double startAngle, double endAngle) const;
	std::vector<Vec2d> tessellateEllipse(const Vec2d& center, double majorRadius, double minorRadius,
...
#endif // GEOMETRICMODELINGPLUGIN_SKETCHRENDERER_H

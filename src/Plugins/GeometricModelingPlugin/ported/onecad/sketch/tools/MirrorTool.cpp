...
EntityID MirrorTool::findLineAtPosition(const Vec2d& pos) const
{
	...
	for (const auto& entity : sketch_->getAllEntities())
	{
		...
		if (startPt && endPt)
		{
			...
			if (lenSq > 1e-10)
			{
				double t = ((testPoint.X() - p1.X()) * dx + (testPoint.Y() - p1.Y()) * dy) / lenSq;
...
Vec2d MirrorTool::mirrorPoint(const Vec2d& point) const
{
	...
	double t = ((point.x - mirrorLineStart_.x) * dx + (point.y - mirrorLineStart_.y) * dy) / lenSq;
...
EntityID MirrorTool::createMirroredEntity(EntityID sourceId)
{
	...
	else if (entity->type() == EntityType::Arc)
	{
		...
		double lineAngle = std::atan2(mirrorLineEnd_.y - mirrorLineStart_.y, mirrorLineEnd_.x - mirrorLineStart_.x);
		...
		return sketch_->addArc(newCenterId, arc->radius(), mirroredStartAngle, mirroredEndAngle);
	}
	else if (entity->type() == EntityType::Ellipse)
	{
		...
		return sketch_->addEllipse(newCenterId, ellipse->majorRadius(), ellipse->minorRadius(), mirroredRotation);
	}

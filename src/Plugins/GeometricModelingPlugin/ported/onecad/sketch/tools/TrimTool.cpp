...
bool trimLineAt(Sketch& sketch, const EntityID& lineId, const Vec2d& click)
{
	...
	auto pointAt = [&](double t) -> Vec2d { return {p1.X() + dx * t, p1.Y() + dy * t}; };
	...
	std::sort(params.begin(), params.end(), [](const ParamPoint& lhs, const ParamPoint& rhs) { return lhs.t < rhs.t; });
...
bool trimArcAt(Sketch& sketch, const EntityID& arcId, const Vec2d& click)
{
	...
	auto pointAtAngle = [&](double angle) -> Vec2d {
		return {center.X() + arc->radius() * std::cos(angle), center.Y() + arc->radius() * std::sin(angle)};
	};
...
bool trimCircleAt(Sketch& sketch, const EntityID& circleId, const Vec2d& click)
{
	...
	for (size_t i = 0; i < params.size(); ++i)
	{
		...
		const double to =
			((i + 1 < params.size()) ? params[i + 1].t : params.front().t + 1.0) * 2.0 * std::numbers::pi_v<double>;
		if (to - from <= kMinSpan)
		{
			continue;
		}

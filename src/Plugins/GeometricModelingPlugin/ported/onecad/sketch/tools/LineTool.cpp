...
{
	qCDebug(logLineTool) << "onMousePress"
						 << "state=" << static_cast<int>(state_) << "pos=" << pos.x << pos.y
						 << "button=" << static_cast<int>(button) << "snapType=" << static_cast<int>(snapResult_.type)
	...
	else if (state_ == State::FirstClick)
	{
		...
		{
			qCDebug(logLineTool) << "reject:too-short"
								 << "length=" << length;
		...
		if (!lineId.empty())
		{
			...
			if (line)
			{
				...
				if (autoConstrainer_ && autoConstrainer_->isEnabled())
				{
					...
					auto constraints =
						autoConstrainer_->inferLineConstraints(startPoint_, endPoint, lineId, *sketch_, context);
					...
					qCDebug(logLineTool) << "auto-constraints"
										 << "inferred=" << constraints.size() << "toApply=" << toApply.size()
										 << "lineId=" << QString::fromStdString(lineId);
...
void LineTool::updateInferredConstraints()
{
	...
	inferredConstraints_ = autoConstrainer_->inferLineConstraints(startPoint_, currentPoint_, {}, *sketch_, context);
	qCDebug(logLineTool) << "updateInferredConstraints:done"
						 << "count=" << inferredConstraints_.size();
}

void LineTool::applyInferredConstraints(const std::vector<InferredConstraint>& constraints, EntityID lineId)
{
	qCDebug(logLineTool) << "applyInferredConstraints"
						 << "lineId=" << QString::fromStdString(lineId) << "count=" << constraints.size();
	if (!sketch_ || lineId.empty())
		return;
	...
	for (const auto& constraint : constraints)
	{
		...
		case ConstraintType::Coincident:
			if (!constraint.entity1.empty())
			{
				...
				if (constraint.entity1 == line->startPointId() || constraint.entity1 == line->endPointId())
				{
					break;
				}
				...
				double tolerance =
					autoConstrainer_ ? autoConstrainer_->getConfig().coincidenceTolerance : constants::SNAP_RADIUS_MM;
...
void LineTool::render(SketchRenderer& renderer)
{
	if (state_ == State::FirstClick)
	{
		...
		if (length > constants::MIN_GEOMETRY_SIZE)
		{
			...
			Vec2d midPoint = {(startPoint_.x + currentPoint_.x) * 0.5, (startPoint_.y + currentPoint_.y) * 0.5};
			dims.push_back({midPoint, std::string(buffer), "line_length", length, "mm"});

			const double angleDeg = normalizeAngleDegrees(std::atan2(dy, dx) * 180.0 / std::numbers::pi);
			char angleBuffer[32];
			std::snprintf(angleBuffer, sizeof(angleBuffer), "%.1f\xC2\xB0", angleDeg);

			Vec2d perp{-dy / length, dx / length};
			Vec2d anglePos = {startPoint_.x + dx * 0.35 + perp.x * 2.0, startPoint_.y + dy * 0.35 + perp.y * 2.0};

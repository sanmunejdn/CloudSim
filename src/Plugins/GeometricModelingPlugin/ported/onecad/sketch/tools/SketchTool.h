 ...
 */
#ifndef GEOMETRICMODELINGPLUGIN_SKETCHTOOL_H
#define GEOMETRICMODELINGPLUGIN_SKETCHTOOL_H
...
public:
     ...
     */
	virtual PreviewDimensionApplyResult applyPreviewDimensionValue(const std::string& /*id*/, double /*value*/)
	{
		return {};
	}
	...
	const std::vector<InferredConstraint>& inferredConstraints() const { return inferredConstraints_; }

	/**
     * @brief Get snapped position (snap point if snapped, raw otherwise)
     * @param rawPos Original cursor position
     */
	Vec2d getSnappedPos(const Vec2d& rawPos) const { return snapResult_.snapped ? snapResult_.position : rawPos; }
...
#endif // GEOMETRICMODELINGPLUGIN_SKETCHTOOL_H

 ...
 */
#ifndef GEOMETRICMODELINGPLUGIN_SKETCHTOOLMANAGER_H
#define GEOMETRICMODELINGPLUGIN_SKETCHTOOLMANAGER_H
...
public:
     ...
     */
	Vec2d snappedPosition() const { return currentSnapResult_.snapped ? currentSnapResult_.position : rawCursorPos_; }

	/**
     * @brief Get current inferred constraints (updated on mouse move)
     */
	const std::vector<InferredConstraint>& currentInferredConstraints() const { return currentInferredConstraints_; }
     ...
     */
	void setExcludeFromSnap(const std::unordered_set<EntityID>& entities) { excludeFromSnap_ = entities; }
...
#endif // GEOMETRICMODELINGPLUGIN_SKETCHTOOLMANAGER_H

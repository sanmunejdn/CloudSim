/// @file SnapPreviewResolver.cpp
/// @brief SnapPreviewResolver 实现
...
{
	return a.snapped == b.snapped && a.type == b.type && std::abs(a.position.x - b.position.x) <= 1e-9 &&
		   std::abs(a.position.y - b.position.y) <= 1e-9 && a.entityId == b.entityId &&
		   a.secondEntityId == b.secondEntityId && a.pointId == b.pointId && a.hasGuide == b.hasGuide &&
		   std::abs(a.guideOrigin.x - b.guideOrigin.x) <= 1e-9 && std::abs(a.guideOrigin.y - b.guideOrigin.y) <= 1e-9 &&
		   a.gridKind == b.gridKind && a.guideIntersectionKind == b.guideIntersectionKind && a.hintText == b.hintText;
}
...
std::optional<size_t> pickNearestGuideCandidateIndex(const std::vector<GuideCandidate>& candidates)
{
	if (candidates.empty())
	{
		return std::nullopt;
...
std::optional<std::pair<size_t, size_t>> pickGuidePairForIntersection(const std::vector<GuideCandidate>& candidates,
																	  const std::optional<Vec2d>& intersectionPos)
{
	...
	for (size_t i = 0; i < candidates.size(); ++i)
	{
		for (size_t j = i + 1; j < candidates.size(); ++j)
		{
			...
			const auto intersection = infiniteLineIntersection(candidates[i].segment, candidates[j].segment);
			...
			if (!bestPair.has_value() || metric < bestPairMetric ||
				(std::abs(metric - bestPairMetric) <= kGuideDistanceTieEps &&
				 std::pair<size_t, size_t>{candidates[i].sourceIndex, candidates[j].sourceIndex} <
					 std::pair<size_t, size_t>{candidates[bestPair->first].sourceIndex,
...
bool hasNonGridGuidePairAt(const std::vector<SnapResult>& allSnaps, const std::vector<GuideCandidate>& candidates,
						   const Vec2d& pos)
{
	for (size_t i = 0; i < candidates.size(); ++i)
	{
...
SnapResult selectBestSnapDeterministically(const SnapManager& snapManager, const Vec2d& pos,
										   const std::vector<SnapResult>& snaps, const Sketch& sketch)
{
												   ...
												   bool includeNonRenderableGuides)
{
	...
	for (size_t i = 0; i < allSnaps.size(); ++i)
	{
		...
		candidates.push_back(
			{.segment = {snap.guideOrigin, snap.position}, .distance = snap.distance, .sourceIndex = i});
	}
	return candidates;
}
...
std::optional<size_t> pickNearestGuideIntersectionSnapIndex(const std::vector<SnapResult>& allSnaps,
															const std::vector<GuideCandidate>& candidates)
{
	...
	for (size_t i = 0; i < allSnaps.size(); ++i)
	{
		...
		if (!bestIndex.has_value() || snap.distance < allSnaps[*bestIndex].distance ||
			(std::abs(snap.distance - allSnaps[*bestIndex].distance) <= kGuideDistanceTieEps && i < *bestIndex))
		{
			bestIndex = i;
		}
	}
...
SnapResult applyGuideFirstSnapPolicy(const SnapResult& fallbackSnap, const std::vector<SnapResult>& allSnaps)
{
	...
	const auto bestGuideIntersection = pickNearestGuideIntersectionSnapIndex(allSnaps, guideCandidates);
												   ...
												   const std::vector<SnapResult>& allSnaps)
{
	...
	if (!resolvedSnap.snapped || allSnaps.empty() || isGuideSuppressedPointSnapType(resolvedSnap.type))
	{
		return activeGuides;
	}
...
SnapInputResolution resolveSnapForInputEvent(const SnapManager& snapManager, const Vec2d& pos, const Sketch& sketch,
											 const std::unordered_set<EntityID>& excludeFromSnap,
											 std::optional<Vec2d> referencePoint, bool preferGuide,
											 bool collectGuideData)
{
	...
	resolution.bestSnap = selectBestSnapDeterministically(snapManager, pos, resolution.allSnaps, sketch);
	...
	const SnapResult guideResolvedSnap = applyGuideFirstSnapPolicy(resolution.bestSnap, resolution.allSnaps);
	...
	if (collectGuideData)
	{
		resolution.activeGuides = buildActiveGuidesForSnap(resolution.resolvedSnap, resolution.allSnaps);
	}

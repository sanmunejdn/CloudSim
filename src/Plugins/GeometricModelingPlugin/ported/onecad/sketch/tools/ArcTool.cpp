...
void ArcTool::onMousePress(const Vec2d& pos, Qt::MouseButton button)
{
	...
	else if (state_ == State::Drawing)
	{
		...
		if (!calculateArcFromThreePoints(startPoint_, middlePoint_, currentPoint_, center, radius, startAngle,
										 endAngle))
		{
...
void ArcTool::render(SketchRenderer& renderer)
{
	...
	{
		// Show preview arc
		renderer.setPreviewArc(previewCenter_, previewRadius_, previewStartAngle_, previewEndAngle_);

		// Show radius dimension
		char buffer[32];
		std::snprintf(buffer, sizeof(buffer), "R%.2f", previewRadius_);

		Vec2d dimPos = {previewCenter_.x + previewRadius_ * 0.7, previewCenter_.y + previewRadius_ * 0.7};
...
bool ArcTool::calculateArcFromThreePoints(const Vec2d& p1, const Vec2d& p2, const Vec2d& p3, Vec2d& center,
										  double& radius, double& startAngle, double& endAngle) const
{
...
{
	previewValid_ = calculateArcFromThreePoints(startPoint_, middlePoint_, currentPoint_, previewCenter_,
												previewRadius_, previewStartAngle_, previewEndAngle_);
}
...
void ArcTool::applyInferredConstraints(EntityID arcId, EntityID centerPointId)
{
	...
	// Infer constraints
	auto constraints = autoConstrainer_->inferArcConstraints(center, arc->radius(), arc->startAngle(), arc->endAngle(),
	...
	for (const auto& c : toApply)
	{
		...
		{
			sketch_->addConstraint(std::make_unique<constraints::TangentConstraint>(*c.entity2, arcId));
		}
	}
}

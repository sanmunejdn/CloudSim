...
void EllipseTool::onMousePress(const Vec2d& pos, Qt::MouseButton button)
{
	...
	else if (state_ == State::Drawing)
	{
		...
		{
			ellipseId = sketch_->addEllipse(centerPointId_, majorRadius_, minorRadius_, rotation_);
		}
		else
		{
			...
			if (!centerId.empty())
			{
				ellipseId = sketch_->addEllipse(centerId, majorRadius_, minorRadius_, rotation_);
			}
		}
...
void EllipseTool::render(SketchRenderer& renderer)
{
	if (state_ == State::FirstClick && majorRadius_ > constants::MIN_GEOMETRY_SIZE)
	{
		...
		Vec2d labelPos = {centerPoint_.x + majorRadius_ * 0.5 * std::cos(rotation_),
	...
	else if (state_ == State::Drawing && majorRadius_ > constants::MIN_GEOMETRY_SIZE)
	{
		...
		Vec2d labelPos1 = {centerPoint_.x + majorRadius_ * 0.5 * std::cos(rotation_),
						   centerPoint_.y + majorRadius_ * 0.5 * std::sin(rotation_)};
		Vec2d labelPos2 = {centerPoint_.x + minorRadius_ * 0.5 * std::cos(rotation_ + M_PI / 2.0),
						   centerPoint_.y + minorRadius_ * 0.5 * std::sin(rotation_ + M_PI / 2.0)};
		renderer.setPreviewDimensions({{labelPos1, label1}, {labelPos2, label2}});
	}
	else
	{

/// @file SketchToolManager.cpp
/// @brief SketchToolManager 实现
...
namespace
{
std::vector<SketchRenderer::GuideLineInfo> toRendererGuides(const std::vector<GuideSegment>& guideSegments)
{
...
bool sameCursorSample(const Vec2d& a, const Vec2d& b)
{
	...
	return std::abs(a.x - b.x) <= kCursorMatchEps && std::abs(a.y - b.y) <= kCursorMatchEps;
}

} // namespace

SketchToolManager::SketchToolManager(QObject* parent) : QObject(parent) {}
...
{
	qCDebug(logSketchToolMgr) << "mousePress"
	...
	if (sketch_)
	{
		const SnapInputResolution snapResolution = resolveSnapForInputEvent(
			snapManager_, pos, *sketch_, excludeFromSnap_, activeTool_->getReferencePoint(), false, false);
		const bool canReusePreviewSnap = hasPreviewCursorSample_ && sameCursorSample(previewCursorPos_, pos) &&
		...
		qCDebug(logSketchToolMgr) << "mousePress:resolution"
								  << "previousPreviewGridConflict=" << previousPreviewHadGridConflict
								  << "gridConflict=" << snapResolution.gridConflict
								  << "allowPreviewCommitMismatch=" << snapResolution.allowPreviewCommitMismatch
...
void SketchToolManager::handleMouseMove(const Vec2d& pos)
{
	...
	if (sketch_)
	{
		const SnapInputResolution snapResolution =
			resolveSnapForInputEvent(snapManager_, pos, *sketch_, excludeFromSnap_, activeTool_->getReferencePoint(),
									 false, renderer_ != nullptr);
		currentSnapResult_ = snapResolution.resolvedSnap;
		previewHadGridConflict_ = snapResolution.gridConflict;
...
{
	qCDebug(logSketchToolMgr) << "mouseRelease"
							  << "pos=" << pos.x << pos.y << "button=" << static_cast<int>(button);
	if (!activeTool_)
	{
		return;
...
void SketchToolManager::renderPreview()
{
	...
	{
		const bool showGuide =
			snapManager_.showGuidePoints() && currentSnapResult_.hasGuide && currentSnapResult_.type != SnapType::Grid;
		std::string hintText = snapManager_.showSnappingHints() ? currentSnapResult_.hintText : std::string();
		renderer_->showSnapIndicator(currentSnapResult_.position, currentSnapResult_.type,
									 currentSnapResult_.guideOrigin, showGuide, hintText);
	...
	if (activeTool_->isActive())
	{
		...
		for (const auto& candidate : anchorCandidates)
		{
			...
			const bool duplicate =
				std::any_of(filteredAnchors.begin(), filteredAnchors.end(),

	...
	else if (state_ == State::FirstClick)
	{
		...
		if (!circleId.empty())
		{
			...
			if (autoConstrainer_ && autoConstrainer_->isEnabled())
			{
				...
				auto constraints =
					autoConstrainer_->inferCircleConstraints(centerPoint_, radius, circleId, *sketch_, context);
				...
				for (const auto& constraint : toApply)
				{
					if (constraint.type == ConstraintType::Coincident)
					{
						...
						if (!centerId.empty() && !constraint.entity1.empty() && centerId != constraint.entity1)
						{
							 ...
							 !constraint.entity2->empty())
					{
						...
						if (!centerId.empty() && !otherCenterId.empty() && centerId != otherCenterId)
						{
...
void CircleTool::render(SketchRenderer& renderer)
{
	if (state_ == State::FirstClick && currentRadius_ > constants::MIN_GEOMETRY_SIZE)
	{
		...
		Vec2d labelPos = {(centerPoint_.x + currentPoint_.x) * 0.5, (centerPoint_.y + currentPoint_.y) * 0.5};

		renderer.setPreviewDimensions({{labelPos, std::string(buffer), "circle_radius", currentRadius_, "mm"}});
	}
	else
	{

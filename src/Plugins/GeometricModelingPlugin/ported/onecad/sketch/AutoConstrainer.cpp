...
AutoConstrainer::AutoConstrainer()
{
	...
	for (int i = static_cast<int>(ConstraintType::Coincident); i <= static_cast<int>(ConstraintType::Symmetric); ++i)
	{
		typeEnabled_[static_cast<ConstraintType>(i)] = true;
	}
}

AutoConstrainer::AutoConstrainer(const AutoConstrainerConfig& config) : config_(config)
{
	// Enable all constraint types by default
	for (int i = static_cast<int>(ConstraintType::Coincident); i <= static_cast<int>(ConstraintType::Symmetric); ++i)
	{
...
std::vector<InferredConstraint> AutoConstrainer::inferConstraints(const Vec2d& point, const Sketch& sketch,
																  const DrawingContext& context) const
{
	...
	qCDebug(logAutoConstrainer) << "inferConstraints:done"
								<< "count=" << results.size();
	return results;
}

std::vector<InferredConstraint> AutoConstrainer::inferLineConstraints(const Vec2d& startPoint, const Vec2d& endPoint,
																	  EntityID lineId, const Sketch& sketch,
																	  const DrawingContext& context) const
{
	if (!config_.enabled)
	{
	...
	qCDebug(logAutoConstrainer) << "inferLineConstraints:start"
								<< "lineId=" << QString::fromStdString(lineId) << "start=" << startPoint.x
								<< startPoint.y << "end=" << endPoint.x << endPoint.y << "length=" << length
								<< "polylineMode=" << context.isPolylineMode << "previousEntity="
								<< (context.previousEntity ? QString::fromStdString(*context.previousEntity)
														   : QStringLiteral("<none>"));
	...
	qCDebug(logAutoConstrainer) << "inferLineConstraints:done"
								<< "count=" << results.size();
	return results;
}

std::vector<InferredConstraint> AutoConstrainer::inferCircleConstraints(const Vec2d& center, double radius,
																		EntityID circleId, const Sketch& sketch,
																		const DrawingContext& context) const
{
	(void)context;
	if (!config_.enabled)
	...
	qCDebug(logAutoConstrainer) << "inferCircleConstraints:start"
								<< "circleId=" << QString::fromStdString(circleId) << "center=" << center.x << center.y
								<< "radius=" << radius;
	...
	qCDebug(logAutoConstrainer) << "inferCircleConstraints:done"
								<< "count=" << results.size();
	return results;
}

std::vector<InferredConstraint> AutoConstrainer::inferArcConstraints(const Vec2d& center, double radius,
																	 double startAngle, double endAngle, EntityID arcId,
																	 const Sketch& sketch,
																	 const DrawingContext& context) const
{
	if (!config_.enabled)
	...
	qCDebug(logAutoConstrainer) << "inferArcConstraints:start"
								<< "arcId=" << QString::fromStdString(arcId) << "center=" << center.x << center.y
								<< "radius=" << radius << "startAngle=" << startAngle << "endAngle=" << endAngle;

	std::vector<InferredConstraint> results;

	// Calculate arc start point
	Vec2d arcStartPoint = {center.x + radius * std::cos(startAngle), center.y + radius * std::sin(startAngle)};
	...
	qCDebug(logAutoConstrainer) << "inferArcConstraints:done"
								<< "count=" << results.size();
	return results;
}

std::vector<InferredConstraint>
AutoConstrainer::filterForAutoApply(const std::vector<InferredConstraint>& constraints) const
{
...
std::optional<InferredConstraint> AutoConstrainer::inferHorizontal(const Vec2d& startPoint, const Vec2d& endPoint,
																   EntityID lineId) const
{
	...
	if (deviation <= config_.horizontalTolerance)
	{
		...
		return InferredConstraint{.type = ConstraintType::Horizontal, .entity1 = lineId, .confidence = confidence};
	}

	return std::nullopt;
}

std::optional<InferredConstraint> AutoConstrainer::inferVertical(const Vec2d& startPoint, const Vec2d& endPoint,
																 EntityID lineId) const
{
	double angle = lineAngle(startPoint, endPoint);
	...
	if (deviation <= config_.verticalTolerance)
	{
		...
		return InferredConstraint{.type = ConstraintType::Vertical, .entity1 = lineId, .confidence = confidence};
	}

	return std::nullopt;
}

std::optional<InferredConstraint> AutoConstrainer::inferCoincident(const Vec2d& point, const Sketch& sketch,
																   EntityID excludeEntity) const
{
	...
	for (const auto& entity : sketch.getAllEntities())
	{
		...
		double distSq = (point.x - ptPos.x) * (point.x - ptPos.x) + (point.y - ptPos.y) * (point.y - ptPos.y);
	...
	if (nearestPoint)
	{
		...
		return InferredConstraint{.type = ConstraintType::Coincident,
								  .entity1 = nearestId,
...
std::optional<InferredConstraint> AutoConstrainer::inferPerpendicular(const Vec2d& line1Start, const Vec2d& line1End,
																	  EntityID line1Id, const Sketch& sketch,
																	  const DrawingContext& context) const
{
	auto considerLine = [&](const SketchLine* line) -> std::optional<InferredConstraint>
	{
		...
		return InferredConstraint{
			.type = ConstraintType::Perpendicular, .entity1 = line1Id, .entity2 = line->id(), .confidence = confidence};
	};
	...
	for (const auto& entity : sketch.getAllEntities())
	{
		if (entity->type() != EntityType::Line)
			continue;
		if (!line1Id.empty() && entity->id() == line1Id)
			continue;
		...
		if (deviation < bestDeviation)
		{
			...
			double confidence = 1.0 - (deviation / config_.perpendicularTolerance);
			best = InferredConstraint{.type = ConstraintType::Perpendicular,
									  .entity1 = line1Id,
									  .entity2 = entity->id(),
									  .confidence = confidence};
		}
...
std::optional<InferredConstraint> AutoConstrainer::inferParallel(const Vec2d& lineStart, const Vec2d& lineEnd,
																 EntityID lineId, const Sketch& sketch,
																 const DrawingContext& context) const
{
	auto considerLine = [&](const SketchLine* line) -> std::optional<InferredConstraint>
	{
		...
		return InferredConstraint{
			.type = ConstraintType::Parallel, .entity1 = lineId, .entity2 = line->id(), .confidence = confidence};
	};
	...
	for (const auto& entity : sketch.getAllEntities())
	{
		if (entity->type() != EntityType::Line)
			continue;
		if (!lineId.empty() && entity->id() == lineId)
			continue;
		...
		if (deviation < bestDeviation)
		{
			...
			best = InferredConstraint{
				.type = ConstraintType::Parallel, .entity1 = lineId, .entity2 = entity->id(), .confidence = confidence};
		}
	}
...
std::optional<InferredConstraint> AutoConstrainer::inferTangent(const Vec2d& arcCenter, const Vec2d& arcStartPoint,
																EntityID arcId, const Sketch& sketch,
																const DrawingContext& context) const
{
	(void)context;
	...
	for (const auto& entity : sketch.getAllEntities())
	{
		...
		Vec2d radialDir = {arcStartPoint.x - arcCenter.x, arcStartPoint.y - arcCenter.y};
		...
		if (dot > std::cos(config_.tangentTolerance))
		{
			...
			if (deviation < bestDeviation)
			{
				...
				best = InferredConstraint{.type = ConstraintType::Tangent,
										  .entity1 = arcId,
										  .entity2 = entity->id(),
										  .confidence = confidence};
			}
...
std::optional<InferredConstraint> AutoConstrainer::inferConcentric(const Vec2d& center, EntityID entityId,
																   const Sketch& sketch) const
{
	...
	for (const auto& entity : sketch.getAllEntities())
	{
		...
		if (dist < bestDist)
		{
			...
			best = InferredConstraint{.type = ConstraintType::Concentric,
									  .entity1 = entityId,
									  .entity2 = entity->id(),
									  .confidence = confidence};
		}
...
std::optional<InferredConstraint> AutoConstrainer::inferEqualRadius(double radius, EntityID entityId,
																	const Sketch& sketch) const
{
	...
	for (const auto& entity : sketch.getAllEntities())
	{
		...
		if (diff < bestDiff)
		{
			...
			best = InferredConstraint{
				.type = ConstraintType::Equal, .entity1 = entityId, .entity2 = entity->id(), .confidence = confidence};
		}
	}
...
double AutoConstrainer::angleBetweenLines(const Vec2d& line1Start, const Vec2d& line1End, const Vec2d& line2Start,
										  const Vec2d& line2End)
{
...
bool AutoConstrainer::areLinesPerpendicular(const Vec2d& line1Start, const Vec2d& line1End, const Vec2d& line2Start,
											const Vec2d& line2End) const
{
	double angle = angleBetweenLines(line1Start, line1End, line2Start, line2End);
	return std::abs(angle - HALF_PI) <= config_.perpendicularTolerance;
}

bool AutoConstrainer::areLinesParallel(const Vec2d& line1Start, const Vec2d& line1End, const Vec2d& line2Start,
									   const Vec2d& line2End) const
{
	double angle = angleBetweenLines(line1Start, line1End, line2Start, line2End);
	// Parallel means angle is 0 or π
	return angle <= config_.parallelTolerance || std::abs(angle - PI) <= config_.parallelTolerance;
}

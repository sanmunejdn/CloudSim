...
SketchEllipse::SketchEllipse() : SketchEntity() {}

SketchEllipse::SketchEllipse(const PointID& centerPointId, double majorRadius, double minorRadius, double rotation)
	: SketchEntity(), m_centerPointId(centerPointId), m_majorRadius(std::max(0.0, majorRadius)),
	  m_minorRadius(std::max(0.0, minorRadius)), m_rotation(rotation)
{
	// Enforce major >= minor
...
void SketchEllipse::getFoci(const gp_Pnt2d& centerPos, gp_Pnt2d& focus1, gp_Pnt2d& focus2) const
{
	...
	double c = std::sqrt(std::max(0.0, m_majorRadius * m_majorRadius - m_minorRadius * m_minorRadius));
...
bool SketchEllipse::isNearWithCenter(const gp_Pnt2d& testPoint, const gp_Pnt2d& centerPos, double tolerance) const
{
...
{
	if (!json.contains("center") || !json.contains("majorRadius") || !json.contains("minorRadius"))
	{
		return false;
	}

	if (!json["center"].isString() || !json["majorRadius"].isDouble() || !json["minorRadius"].isDouble())
	{
		return false;
	}
	...
	qCDebug(logSketchEllipse) << "deserialize:done"
							  ...
							  << "center=" << QString::fromStdString(m_centerPointId) << "majorRadius=" << m_majorRadius

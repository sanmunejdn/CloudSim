...
std::optional<Vec2d> infiniteLineIntersection(const Vec2d& p1, const Vec2d& p2, const Vec2d& p3, const Vec2d& p4)
{
...
SnapManager::SnapManager()
{
	...
	for (int i = static_cast<int>(SnapType::Vertex); i <= static_cast<int>(SnapType::ActiveLayer3D); ++i)
	{
...
SnapResult SnapManager::findBestSnap(const Vec2d& cursorPos, const Sketch& sketch,
									 ...
									 std::optional<Vec2d> referencePoint) const
{
	qCDebug(logSnapManager) << "findBestSnap:start"
							<< "cursor=" << cursorPos.x << cursorPos.y << "excludeCount=" << excludeEntities.size()
	...
	if (coLocated.size() > 1)
	{
		...
		qCDebug(logSnapManager) << "findBestSnap:ambiguity-detected"
								<< "candidateCount=" << coLocated.size() << "bestType=" << static_cast<int>(best.type);
	}

	qCDebug(logSnapManager) << "findBestSnap:resolved"
							<< "type=" << static_cast<int>(best.type) << "distance=" << best.distance
							<< "gridConflict=" << lastGridConflictDetected_
							<< "entity=" << QString::fromStdString(best.entityId);
	return best;
}

std::vector<SnapResult> SnapManager::findAllSnaps(const Vec2d& cursorPos, const Sketch& sketch,
												  const std::unordered_set<EntityID>& excludeEntities,
												  std::optional<Vec2d> referencePoint) const
{
	if (!enabled_)
	...
	qCDebug(logSnapManager) << "findAllSnaps:start"
							<< "cursor=" << cursorPos.x << cursorPos.y << "snapRadius=" << snapRadius_
		...
		for (size_t i = 0; i < guideCandidates.size(); ++i)
		{
			for (size_t j = i + 1; j < guideCandidates.size(); ++j)
			{
				...
				const auto intersection =
					infiniteLineIntersection(guideCandidates[i].guideOrigin, guideCandidates[i].position,
											 guideCandidates[j].guideOrigin, guideCandidates[j].position);
				...
				results.push_back({.snapped = true,
...
SnapResult SnapManager::selectBestSnapFromCandidates(const Vec2d& cursorPos, const Sketch& sketch,
...
void SnapManager::applyGridStickyStateFromSelection(const SnapResult& selected,
...
std::optional<SnapResult> SnapManager::buildStickyGridCandidate(const Vec2d& cursorPos) const
{
	...
	case SnapResult::GridCandidateKind::Crossing:
	{
		...
		return SnapResult{.snapped = true,
						  .type = SnapType::Grid,
...
std::optional<SnapResult> SnapManager::pickBestGridCandidate(const Vec2d& cursorPos,
															 const std::vector<SnapResult>& snaps) const
{
	auto classifyGridKind = [&](const SnapResult& snap)
	{
		...
		return dx > dy ? SnapResult::GridCandidateKind::AxisX : SnapResult::GridCandidateKind::AxisY;
	};
...
bool SnapManager::isGridConflict(const std::vector<SnapResult>& snaps, const SnapResult& selectedGrid) const
{
	...
	for (const auto& snap : snaps)
	{
		...
		if (isPointType(snap.type) || isNonGridGeometryType(snap) || isMixedGridGuideIntersection(snap) ||
			snap.type == SnapType::SketchGuide || snap.type == SnapType::Horizontal ||
			snap.type == SnapType::Vertical || snap.type == SnapType::ActiveLayer3D)
		{
			return true;
		}
	}
...
SnapResult SnapManager::selectBestSnapCandidate(const Vec2d& cursorPos, const Sketch& sketch,
												const std::vector<SnapResult>& snaps) const
{
	auto pickDeterministic = [&](const std::vector<SnapResult>& candidates)
	{
		...
		for (const auto& snap : sorted)
		{
			...
			if (candidateOrder < currentOrder || (candidateOrder == currentOrder && snap < *bestVertex))
			{
				bestVertex = &snap;
			}
		}
	...
	if (gridBest.has_value() && nonGridBest.has_value())
	{
		...
		{
			if (isPointType(nonGridBest->type) || isNonGridGeometryType(*nonGridBest) ||
				nonGridBest->type == SnapType::SketchGuide)
			{
				selected = *nonGridBest;
			}
			else if (isMixedGridGuideIntersection(*nonGridBest) || nonGridBest->type == SnapType::Horizontal ||
					 nonGridBest->type == SnapType::Vertical || nonGridBest->type == SnapType::ActiveLayer3D)
			{
				selected = *gridBest;
			}
			else
...
bool SnapManager::shouldConsiderEntity(const EntityID& entityId, const std::unordered_set<EntityID>* candidateSet) const
{
	if (!candidateSet)
	{
		return true;
...
void SnapManager::findVertexSnaps(const Vec2d& cursorPos, const Sketch& sketch,
								  const std::unordered_set<EntityID>& excludeEntities,
								  const std::unordered_set<EntityID>* candidateSet, double radiusSq,
								  std::vector<SnapResult>& results) const
{
	for (const auto& entity : sketch.getAllEntities())
	{
		if (!shouldConsiderEntity(entity->id(), candidateSet))
			continue;
		if (excludeEntities.count(entity->id()))
			continue;
		if (entity->type() != EntityType::Point)
			continue;
...
void SnapManager::findEndpointSnaps(const Vec2d& cursorPos, const Sketch& sketch,
									const std::unordered_set<EntityID>& excludeEntities,
									const std::unordered_set<EntityID>* candidateSet, double radiusSq,
									std::vector<SnapResult>& results) const
{
...
void SnapManager::findMidpointSnaps(const Vec2d& cursorPos, const Sketch& sketch,
									const std::unordered_set<EntityID>& excludeEntities,
									const std::unordered_set<EntityID>* candidateSet, double radiusSq,
									std::vector<SnapResult>& results) const
{
...
void SnapManager::findCenterSnaps(const Vec2d& cursorPos, const Sketch& sketch,
								  const std::unordered_set<EntityID>& excludeEntities,
								  const std::unordered_set<EntityID>* candidateSet, double radiusSq,
								  std::vector<SnapResult>& results) const
{
...
void SnapManager::findQuadrantSnaps(const Vec2d& cursorPos, const Sketch& sketch,
									const std::unordered_set<EntityID>& excludeEntities,
									const std::unordered_set<EntityID>* candidateSet, double radiusSq,
									std::vector<SnapResult>& results) const
{
	...
	for (const auto& entity : sketch.getAllEntities())
	{
		...
		if (entity->type() == EntityType::Circle)
		{
			...
			{
				Vec2d quadPt = {center.x + r * std::cos(angle), center.y + r * std::sin(angle)};
...
void SnapManager::findIntersectionSnaps(const Vec2d& cursorPos, const Sketch& sketch,
										const std::unordered_set<EntityID>& excludeEntities,
										const std::unordered_set<EntityID>* candidateSet, double radiusSq,
										std::vector<SnapResult>& results) const
{
	...
	for (const auto& entity : sketch.getAllEntities())
	{
		...
		if (entity->type() == EntityType::Line || entity->type() == EntityType::Arc ||
			entity->type() == EntityType::Circle || entity->type() == EntityType::Ellipse)
		{
	...
	for (size_t i = 0; i < entities.size(); ++i)
	{
		...
		{
			auto intersections = findEntityIntersections(entities[i], entities[j], sketch);
...
void SnapManager::findOnCurveSnaps(const Vec2d& cursorPos, const Sketch& sketch,
								   const std::unordered_set<EntityID>& excludeEntities,
								   const std::unordered_set<EntityID>* candidateSet, double radiusSq,
								   std::vector<SnapResult>& results) const
{
	for (const auto& entity : sketch.getAllEntities())
	{
		...
		if (entity->type() == EntityType::Line)
		{
			...
			nearestPt = nearestPointOnLine(cursorPos, toVec2d(startPt->position()), toVec2d(endPt->position()));
			found = true;
		}
		else if (entity->type() == EntityType::Circle)
		{
			...
			nearestPt = nearestPointOnCircle(cursorPos, toVec2d(centerPt->position()), circle->radius());
			found = true;
		}
		else if (entity->type() == EntityType::Arc)
		{
		...
		else if (entity->type() == EntityType::Ellipse)
		{
			...
			auto pointAt = [&](double t) { return toVec2d(ellipse->pointAtParameter(center, t)); };
			auto distAt = [&](double t) { return distanceSquared(cursorPos, pointAt(t)); };
...
void SnapManager::findGridSnaps(const Vec2d& cursorPos, std::vector<SnapResult>& results) const
{
	if (gridSize_ <= 0.0)
	{
		return;
...
void SnapManager::findPerpendicularSnaps(const Vec2d& cursorPos, const Sketch& sketch,
										 const std::unordered_set<EntityID>& excludeEntities, double radiusSq,
										 std::vector<SnapResult>& results) const
{
	...
	for (const auto& entity : sketch.getAllEntities())
	{
		...
		if (entity->type() == EntityType::Line)
		{
			...
			const double t = ((cursorPos.x - startPos.x) * lineDir.x + (cursorPos.y - startPos.y) * lineDir.y) / len2;
			if (t < 0.0 || t > 1.0)
				continue;

			foot = {startPos.x + t * lineDir.x, startPos.y + t * lineDir.y};
			found = true;
		}
		else if (entity->type() == EntityType::Circle)
		{
			...
			foot = {center.x + radius * dx / len, center.y + radius * dy / len};
			found = true;
		}
		else if (entity->type() == EntityType::Arc)
		{
			...
			const double len = std::sqrt(len2);
			const Vec2d onCircle{center.x + radius * dx / len, center.y + radius * dy / len};
		...
		if (distSq <= radiusSq)
		{
			SnapResult result{.snapped = true,
...
void SnapManager::findTangentSnaps(const Vec2d& cursorPos, const Sketch& sketch,
								   const std::unordered_set<EntityID>& excludeEntities, double radiusSq,
								   std::vector<SnapResult>& results) const
{
	constexpr double kGeomEps = 1e-6;
	...
	for (const auto& entity : sketch.getAllEntities())
	{
		...
		const Vec2d base{center.x + vp.x * (r2 / c2), center.y + vp.y * (r2 / c2)};
...
void SnapManager::findHorizontalSnaps(const Vec2d& cursorPos, const Sketch& sketch,
									  const std::unordered_set<EntityID>& excludeEntities,
									  std::vector<SnapResult>& results) const
{
	...
	for (const auto& entity : sketch.getAllEntities())
	{
		...
		if (entity->type() == EntityType::Point)
		{
			const auto* point = static_cast<const SketchPoint*>(entity.get());
			points.push_back(
				{.position = toVec2d(point->position()), .entityId = entity->id(), .pointId = entity->id()});
		}
		else if (entity->type() == EntityType::Line)
		{
			...
			{
				points.push_back({.position = toVec2d(startPt->position()),
								  .entityId = entity->id(),
								  .pointId = line->startPointId()});
			}
			...
			{
				points.push_back(
					{.position = toVec2d(endPt->position()), .entityId = entity->id(), .pointId = line->endPointId()});
			}

			points.push_back({.position = {(startPt->position().X() + endPt->position().X()) * 0.5,
		...
		{
			const auto* circle = static_cast<const SketchCircle*>(entity.get());
			if (excludeEntities.count(circle->centerPointId()))
				continue;
			const auto* centerPt = sketch.getEntityAs<SketchPoint>(circle->centerPointId());
			if (!centerPt)
				continue;

			points.push_back({.position = toVec2d(centerPt->position()),
							  .entityId = entity->id(),
		...
		{
			const auto* arc = static_cast<const SketchArc*>(entity.get());
			if (excludeEntities.count(arc->centerPointId()))
				continue;
			const auto* centerPt = sketch.getEntityAs<SketchPoint>(arc->centerPointId());
			if (!centerPt)
				continue;
			...
			points.push_back(
				{.position = toVec2d(centerPt->position()), .entityId = entity->id(), .pointId = arc->centerPointId()});
		}
		else if (entity->type() == EntityType::Ellipse)
		{
			const auto* ellipse = static_cast<const SketchEllipse*>(entity.get());
			if (excludeEntities.count(ellipse->centerPointId()))
				continue;
			const auto* centerPt = sketch.getEntityAs<SketchPoint>(ellipse->centerPointId());
			if (!centerPt)
				continue;
...
void SnapManager::findVerticalSnaps(const Vec2d& cursorPos, const Sketch& sketch,
									const std::unordered_set<EntityID>& excludeEntities,
									std::vector<SnapResult>& results) const
{
	struct CandidatePoint
...
void SnapManager::findGuideSnaps(const Vec2d& cursorPos, const Sketch& sketch,
								 const std::unordered_set<EntityID>& excludeEntities, double radiusSq,
								 std::vector<SnapResult>& results) const
{
	for (const auto& entity : sketch.getAllEntities())
	{
		...
		const double t = ((cursorPos.x - lineStart.x) * d.x + (cursorPos.y - lineStart.y) * d.y) / len2;
		if (t >= 0.0 && t <= 1.0)
			continue;

		if (std::abs(t) > 4.0 || std::abs(t - 1.0) > 3.0)
			continue;

		const Vec2d projected{lineStart.x + t * d.x, lineStart.y + t * d.y};
...
void SnapManager::findAngularSnap(const Vec2d& cursorPos, const Vec2d& referencePoint, double radiusSq,
								  std::vector<SnapResult>& results) const
{
	...
	const Vec2d snappedPos{referencePoint.x + dist * std::cos(thetaSnapped),
...
Vec2d SnapManager::nearestPointOnLine(const Vec2d& point, const Vec2d& lineStart, const Vec2d& lineEnd)
{
...
Vec2d SnapManager::nearestPointOnCircle(const Vec2d& point, const Vec2d& center, double radius)
{
...
std::optional<Vec2d> SnapManager::lineLineIntersection(const Vec2d& p1, const Vec2d& p2, const Vec2d& p3,
													   const Vec2d& p4)
{
...
std::vector<Vec2d> SnapManager::lineCircleIntersection(const Vec2d& lineStart, const Vec2d& lineEnd,
													   const Vec2d& center, double radius)
{
	...
	if (t1 >= 0.0 && t1 <= 1.0)
	{
		result.push_back({lineStart.x + t1 * dx, lineStart.y + t1 * dy});
	}

	if (t2 >= 0.0 && t2 <= 1.0 && std::abs(t2 - t1) > 1e-12)
	{
		result.push_back({lineStart.x + t2 * dx, lineStart.y + t2 * dy});
	}

	return result;
}

std::vector<Vec2d> SnapManager::circleCircleIntersection(const Vec2d& center1, double radius1, const Vec2d& center2,
														 double radius2)
{
	std::vector<Vec2d> result;
	...
	if (d > radius1 + radius2 + 1e-12 || d < std::abs(radius1 - radius2) - 1e-12 || d < 1e-12)
	{
		return result;
	}
...
void SnapManager::findExternalSnaps(const Vec2d& cursorPos, double radiusSq, std::vector<SnapResult>& results) const
{
	// Snap to 3D Points
	for (const auto& pt : extPoints_)
	{
		...
		{
			results.push_back(
				{.snapped = true, .type = SnapType::ActiveLayer3D, .position = pt, .distance = std::sqrt(distSq)});
		}
	}
	...
	for (const auto& line : extLines_)
	{
		...
		{
			results.push_back(
				{.snapped = true, .type = SnapType::ActiveLayer3D, .position = nearest, .distance = std::sqrt(distSq)});
		}
	}
}

std::vector<Vec2d> SnapManager::findEntityIntersections(const SketchEntity* e1, const SketchEntity* e2,
														const Sketch& sketch) const
{
	...
	auto getEllipseData = [&sketch](const SketchEllipse* ellipse, Vec2d& center, double& majorRadius,
									double& minorRadius, double& rotation) -> bool
	{
		const auto* centerPt = sketch.getEntityAs<SketchPoint>(ellipse->centerPointId());
		if (!centerPt)
		{
	...
	auto lineEllipseIntersection = [](const Vec2d& lineStart, const Vec2d& lineEnd, const Vec2d& center,
									  double majorRadius, double minorRadius, double rotation) -> std::vector<Vec2d>
	{
		...
		auto toEllipseLocal = [&](const Vec2d& worldPt)
		{
			...
			return Vec2d{dx * cosR + dy * sinR, -dx * sinR + dy * cosR};
		};

		auto fromEllipseLocal = [&](const Vec2d& localPt) {
			return Vec2d{center.x + localPt.x * cosR - localPt.y * sinR,
		...
		auto appendIfOnSegment = [&](double t)
		{
			...
			const Vec2d hitLocal{p0Local.x + t * (p1Local.x - p0Local.x), p0Local.y + t * (p1Local.y - p0Local.y)};

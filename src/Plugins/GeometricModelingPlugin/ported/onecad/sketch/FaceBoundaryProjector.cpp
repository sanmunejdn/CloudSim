/// @file FaceBoundaryProjector.cpp
/// @brief FaceBoundaryProjector 实现
...
PointResolveResult findOrCreatePoint(Sketch& sketch, const Vec2d& position, double tolerance,
...
bool lineExists(const Sketch& sketch, const EntityID& startId, const EntityID& endId, double tolerance)
{
	...
	for (const auto& entity : sketch.getAllEntities())
	{
		...
		const bool direct = samePoint(*startPos, *aPos, tolerance) && samePoint(*endPos, *bPos, tolerance);
		const bool reversed = samePoint(*startPos, *bPos, tolerance) && samePoint(*endPos, *aPos, tolerance);
		if (direct || reversed)
		{
			return true;
		}
...
bool circleExists(const Sketch& sketch, const Vec2d& center, double radius, double pointTolerance,
				  double radiusTolerance)
{
	for (const auto& entity : sketch.getAllEntities())
	{
		...
		if (samePoint(*centerPos, center, pointTolerance) && nearlyEqual(circle->radius(), radius, radiusTolerance))
		{
			return true;
		}
	}
...
bool arcExists(const Sketch& sketch, const Vec2d& center, double radius, double startAngle, double endAngle,
...
SegmentResult addLineSegment(Sketch& sketch, const Vec2d& start, const Vec2d& end,
...
std::vector<Vec2d> sampleEdgePolyline(const BRepAdaptor_Curve& curve, const Sketch& sketch, int segments)
{
...
FaceBoundaryProjector::ProjectionResult
FaceBoundaryProjector::projectFaceBoundaries(const TopoDS_Face& face, Sketch& sketch, const Options& options)
{
	...
	for (TopExp_Explorer wireExp(face, TopAbs_WIRE); wireExp.More(); wireExp.Next())
	{
		...
		qCDebug(logFaceBoundaryProjector) << "projectFaceBoundaries:wire-start"
										  << "wireIndex=" << wireCount << "wireClosed=" << wireIsClosed;
		...
		while (edgeExp.More())
		{
			...
			else if (curveType == GeomAbs_Circle && options.preferExactPrimitives)
			{
				...
				if (radius > kPointEpsilon)
				{
					...
					const bool isFullCircle =
						edge.Closed() || nearlyEqual(span, 2.0 * std::numbers::pi_v<double>, 1e-3);

					if (isFullCircle)
					{
						const gp_Pnt center3d = circle.Location();
						const Vec2d center = sketch.toSketch({center3d.X(), center3d.Y(), center3d.Z()});
						const auto centerRes = findOrCreatePoint(sketch, center, options.pointMergeTolerance, result);
						if (!centerRes.id.empty())
						{
							...
							if (!circleExists(sketch, center, radius, options.pointMergeTolerance,
					...
					else
					{
						...
						if (!centerRes.id.empty())
						{
							...
							if (!useFallback)
							{
								...
								const double tangentX = tangent3d.X() * plane.xAxis.x + tangent3d.Y() * plane.xAxis.y +
														tangent3d.Z() * plane.xAxis.z;
								const double tangentY = tangent3d.X() * plane.yAxis.x + tangent3d.Y() * plane.yAxis.y +
							...
							if (!useFallback)
							{
								...
								{
									if (!arcExists(sketch, center, radius, arcStart, arcEnd,
												   options.pointMergeTolerance, options.radiusTolerance))
									{
										const EntityID arcId =
											sketch.addArc(centerRes.id, radius, arcStart, arcEnd, false);
										if (!arcId.empty())
										{
						...
						if (!insertedExactArc)
						{
							...
							{
								const auto seg =
									addLineSegment(sketch, sampled[i], sampled[i + 1], options, result, pointsToFix);
								if (!seg.valid)
								{
									continue;
								}
			...
			else
			{
				...
				for (size_t i = 0; i + 1 < sampled.size(); ++i)
				{
					const auto seg = addLineSegment(sketch, sampled[i], sampled[i + 1], options, result, pointsToFix);
					if (!seg.valid)
					{
						continue;
					}
		...
		if (wireClosedByCircle)
		{
			result.hasClosedBoundary = true;
				 ...
				 ((wireIsClosed && !firstPointId.empty()) || (!firstPointId.empty() && firstPointId == lastPointId)))
		{
			result.hasClosedBoundary = true;
		}
		qCDebug(logFaceBoundaryProjector)
			<< "projectFaceBoundaries:wire-done"
			<< "wireIndex=" << wireCount << "wireCurveCount=" << wireCurveCount
			<< "wireClosedByCircle=" << wireClosedByCircle << "firstPointId=" << QString::fromStdString(firstPointId)
	...
	qCInfo(logFaceBoundaryProjector) << "projectFaceBoundaries:done"
									 << "entityCount=" << sketch.getAllEntities().size() << "wireCount=" << wireCount
									 << "success=" << result.success << "hasClosedBoundary=" << result.hasClosedBoundary

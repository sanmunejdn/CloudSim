...
Vec3d colorForState(SelectionState state, bool isConstruction, bool hasError, const SketchColors& colors)
{
	if (hasError)
	{
...
float lineWidthForState(SelectionState state, bool isConstruction, const SketchRenderStyle& style)
{
	switch (state)
	{
	case SelectionState::Selected:
...
void appendSolidPolyline(std::vector<float>& data, const std::vector<Vec2d>& vertices, const Vec3d& color)
{
	if (vertices.size() < 2)
	{
		return;
...
void appendDashedPolyline(std::vector<float>& data, const std::vector<Vec2d>& vertices, const Vec3d& color,
...
bool segmentsIntersect(const Vec2d& a1, const Vec2d& a2, const Vec2d& b1, const Vec2d& b2)
{
	...
	if (d1 == 0.0 && onSegment(a1, a2, b1))
		return true;
	if (d2 == 0.0 && onSegment(a1, a2, b2))
		return true;
	if (d3 == 0.0 && onSegment(b1, b2, a1))
		return true;
	if (d4 == 0.0 && onSegment(b1, b2, a2))
		return true;

	return false;
}

bool segmentIntersectsPolygon(const Vec2d& a, const Vec2d& b, const std::vector<Vec2d>& polygon, double tolerance)
{
	if (polygon.size() < 2)
	{
		return false;
...
bool polygonsIntersect(const std::vector<Vec2d>& poly1, const std::vector<Vec2d>& poly2)
{
...
bool pointInTriangle(const Vec2d& p, const Vec2d& a, const Vec2d& b, const Vec2d& c, double tolerance)
{
...
bool triangulateSimplePolygon(const std::vector<Vec2d>& polygonInput, std::vector<Vec2d>& outTriangles)
{
	outTriangles.clear();
...
std::vector<Vec2d> mergePolygons(const std::vector<Vec2d>& outer, int outerIndex, const std::vector<Vec2d>& hole,
								 int holeIndex)
{
...
bool triangulatePolygonWithHoles(const std::vector<Vec2d>& outerInput,
								 const std::vector<std::vector<Vec2d>>& holesInput, std::vector<Vec2d>& outTriangles)
{
	outTriangles.clear();
...
bool polygonContainsPolygon(const std::vector<Vec2d>& outer, const std::vector<Vec2d>& inner, double tolerance)
{
	...
	{
		if (p.x < outerMin.x - tolerance || p.y < outerMin.y - tolerance || p.x > outerMax.x + tolerance ||
			p.y > outerMax.y + tolerance)
		{
			return false;
		}
	}
...
bool buildTrianglesFromFace(const loop::Face& faceDef, const Sketch& sketch, std::vector<Vec2d>& outTriangles)
{
	outTriangles.clear();
...
public:
	...
	void buildVBOs(const std::vector<EntityRenderData>& entities,
				   const std::vector<SketchRenderer::RegionRenderData>& regions, const SketchRenderStyle& style,
				   RegionDisplayMode regionDisplayMode, bool suppressRegionFill,
				   const std::unordered_map<EntityID, SelectionState>& selections,
				   const std::unordered_set<std::string>& selectedRegions, std::optional<std::string> hoverRegion,
				   EntityID hoverEntity, const Viewport& viewport, double pixelScale,
				   const std::vector<ConstraintRenderData>& constraints, const std::vector<InferredConstraint>& ghosts,
				   bool snapActive, SnapType snapType, const Vec2d& snapPos, float snapSize, const Vec3d& snapColor,
				   const Vec2d& snapGuideOrigin, bool snapHasGuide, const std::vector<Vec2d>& anchorIndicators,
	...
	void renderPreview(const QMatrix4x4& mvp, const std::vector<Vec2d>& vertices, const Vec3d& color, float lineWidth);
...
bool SketchRendererImpl::initialize()
{
	...
	auto cleanupOnFailure = [this]()
	{
		lineShader_.reset();
		pointShader_.reset();
		if (lineVAO_ && lineVAO_->isCreated())
			lineVAO_->destroy();
		if (lineVBO_ && lineVBO_->isCreated())
			lineVBO_->destroy();
		if (constructionLineVAO_ && constructionLineVAO_->isCreated())
			constructionLineVAO_->destroy();
		if (constructionLineVBO_ && constructionLineVBO_->isCreated())
			constructionLineVBO_->destroy();
		if (highlightLineVAO_ && highlightLineVAO_->isCreated())
			highlightLineVAO_->destroy();
		if (highlightLineVBO_ && highlightLineVBO_->isCreated())
			highlightLineVBO_->destroy();
		if (guideLineVAO_ && guideLineVAO_->isCreated())
			guideLineVAO_->destroy();
		if (guideLineVBO_ && guideLineVBO_->isCreated())
			guideLineVBO_->destroy();
		if (regionVAO_ && regionVAO_->isCreated())
			regionVAO_->destroy();
		if (regionVBO_ && regionVBO_->isCreated())
			regionVBO_->destroy();
		if (pointVAO_ && pointVAO_->isCreated())
			pointVAO_->destroy();
		if (pointVBO_ && pointVBO_->isCreated())
			pointVBO_->destroy();
		if (previewVAO_ && previewVAO_->isCreated())
			previewVAO_->destroy();
		if (previewVBO_ && previewVBO_->isCreated())
			previewVBO_->destroy();
		lineVAO_.reset();
		lineVBO_.reset();
...
void SketchRendererImpl::buildVBOs(
	const std::vector<EntityRenderData>& entities, const std::vector<SketchRenderer::RegionRenderData>& regions,
	const SketchRenderStyle& style, RegionDisplayMode regionDisplayMode, bool suppressRegionFill,
	const std::unordered_map<EntityID, SelectionState>& selections,
	const std::unordered_set<std::string>& selectedRegions, std::optional<std::string> hoverRegion,
	EntityID hoverEntity, const Viewport& viewport, double pixelScale,
	const std::vector<ConstraintRenderData>& constraints, const std::vector<InferredConstraint>& ghosts,
	...
	const std::vector<SketchRenderer::GuideLineInfo>& dragGuides)
{
	...
	if (!suppressRegionFill)
	{
		for (size_t i = 0; i < regions.size(); ++i)
		{
			...
			float alpha = (regionDisplayMode == RegionDisplayMode::AssistAll) ? style.regionOpacity : 0.0f;
	...
	for (const auto& entity : entities)
	{
		...
		Vec3d color = colorForState(selState, entity.isConstruction, entity.hasError, style.colors);
		...
		else
		{
			...
			bool isHighlight = (selState == SelectionState::Selected || selState == SelectionState::Dragging ||
								selState == SelectionState::Hover);
			if (entity.isConstruction)
			{
				appendDashedPolyline(constructionLineData, entity.vertices, color, dashLength, gapLength);
			}
			else if (isHighlight)
			{
			...
			{
				const Vec2d mid = {(entity.vertices[0].x + entity.vertices[1].x) * 0.5,
	...
	for (const auto& icon : constraints)
	{
		...
		{
			color = {style.colors.selectedGeometry.x * 0.7 + style.colors.constraintIcon.x * 0.3,
	...
	auto clipGuideRayToViewport = [&](const Vec2d& origin, const Vec2d& target, Vec2d& farStart, Vec2d& farEnd)
	{
	...
	if (!dragGuides.empty())
	{
		...
		for (const auto& guide : dragGuides)
		{
			...
			appendDashedPolyline(guideLineData, {farStart, farEnd}, kDragGuideColor, guideDash, guideGap);
		}
	}
	...
	if (snapActive)
	{
		...
		const double pointGuideZoomFade = std::clamp(1.0 / std::sqrt(std::max(pixelScale, 1e-9) * 12.0), 0.45, 1.0);
		const Vec3d guideColor =
			pointSnap ? Vec3d{kGuideColorFaint.x * pointGuideZoomFade, kGuideColorFaint.y * pointGuideZoomFade,
		...
		if (!activeGuides.empty())
		{
			...
			for (const auto& guide : activeGuides)
			{
				...
				appendDashedPolyline(guideLineData, {farStart, farEnd}, guideColor, guideDash, guideGap);
				count++;
			}
		}
		...
		const Vec3d crossColor =
			pointSnap ? guideColor : Vec3d{kGuideColor.x * 0.65, kGuideColor.y * 0.65, kGuideColor.z * 0.65};
		const double crossHalf = 5.0 * std::max(pixelScale, 1e-9);
		const bool guideCrossSnap = (snapType == SnapType::Intersection && snapHasGuide && activeGuides.size() >= 2);
		if (guideCrossSnap)
		{
			appendSegment(guideLineData, {snapPos.x - crossHalf, snapPos.y}, {snapPos.x + crossHalf, snapPos.y},
						  crossColor);
			appendSegment(guideLineData, {snapPos.x, snapPos.y - crossHalf}, {snapPos.x, snapPos.y + crossHalf},
						  crossColor);
		}
	...
	if (!guideLineData.empty())
	{
		guideLineVAO_->bind();
		guideLineVBO_->bind();
		guideLineVBO_->allocate(guideLineData.data(), static_cast<int>(guideLineData.size() * sizeof(float)));
...
{
	if (!initialized_ || pointVertexCount_ == 0)
		return;
...
void SketchRendererImpl::renderPreview(const QMatrix4x4& mvp, const std::vector<Vec2d>& vertices, const Vec3d& color,
									   float lineWidth)
{
	if (!initialized_ || vertices.size() < 2)
		return;
		...
		case EntityType::Arc:
		{
			...
			if (arc)
			{
				...
				if (center)
				{
					...
					data.vertices = tessellateArc(c, arc->radius(), arc->startAngle(), arc->endAngle());
				}
			}
			break;
		}
		case EntityType::Circle:
		{
			...
			if (circle)
			{
				...
				if (center)
				{
					Vec2d c{center->x(), center->y()};
					// Full circle: 0 to 2π
					data.vertices = tessellateArc(c, circle->radius(), 0.0, 2.0 * std::numbers::pi);
				}
			}
			break;
		}
		case EntityType::Ellipse:
		{
			...
			if (ellipse)
			{
				...
				if (center)
				{
					Vec2d c{center->x(), center->y()};
					data.vertices =
						tessellateEllipse(c, ellipse->majorRadius(), ellipse->minorRadius(), ellipse->rotation());
				}
			}
			break;
		}
...
void SketchRenderer::updateConstraints()
{
	...
	std::unordered_set<ConstraintID> conflicting(conflictingConstraints_.begin(), conflictingConstraints_.end());
...
void SketchRenderer::updateRegions()
{
	...
	for (auto& region : regionRenderData_)
	{
		...
		for (const auto& candidate : regionRenderData_)
		{
			...
			if (!polygonContainsPolygon(candidate.outerPolygon, region.outerPolygon, kGeometryEpsilon))
			{
				continue;
			}
			containmentDepth++;
...
std::vector<RegionPickHit> SketchRenderer::pickRegionCandidates(const Vec2d& sketchPos) const
{
	...
	for (const auto& region : regionRenderData_)
	{
		if (sketchPos.x < region.boundsMin.x || sketchPos.x > region.boundsMax.x || sketchPos.y < region.boundsMin.y ||
			sketchPos.y > region.boundsMax.y)
		{
			continue;
		}
	...
	std::sort(hits.begin(), hits.end(),
			  [](const RegionPickHit& a, const RegionPickHit& b)
			  {
				  if (a.area != b.area)
				  {
...
std::optional<RegionPickHit> SketchRenderer::getRegionPickHit(const std::string& regionId) const
{
	...
	for (const auto& region : regionRenderData_)
	{
		...
		{
			return RegionPickHit{region.id, region.area, region.containmentDepth, !region.holes.empty()};
		}
	}
	return std::nullopt;
}
...
void SketchRenderer::render(const QMatrix4x4& viewMatrix, const QMatrix4x4& projMatrix)
{
	...
	{
		impl_->renderPreview(mvp, preview_.vertices, style_.colors.previewGeometry, style_.previewLineWidth);
	}
}
...
void SketchRenderer::setPreviewArc(const Vec2d& center, double radius, double startAngle, double endAngle)
{
	preview_.active = true;
...
void SketchRenderer::setPreviewEllipse(const Vec2d& center, double majorRadius, double minorRadius, double rotation)
{
	preview_.active = true;
...
void SketchRenderer::showSnapIndicator(const Vec2d& pos, SnapType type, const Vec2d& guideOrigin, bool hasGuide,
...
std::vector<EntityPickHit> SketchRenderer::pickEntities(const Vec2d& screenPos, double tolerance) const
{
	...
	for (const auto& data : entityRenderData_)
	{
		...
		else
		{
			...
			for (size_t i = 0; i + 1 < data.vertices.size(); ++i)
			{
				...
				double t = std::clamp(((screenPos.x - p1.x) * dx + (screenPos.y - p1.y) * dy) / lenSq, 0.0, 1.0);
	...
	std::sort(hits.begin(), hits.end(),
			  [](const EntityPickHit& a, const EntityPickHit& b) { return a.distance < b.distance; });

	return hits;
}

ConstraintID SketchRenderer::pickConstraint(const Vec2d& screenPos, double tolerance) const
{
...
std::vector<Vec2d> SketchRenderer::tessellateArc(const Vec2d& center, double radius, double startAngle,
												 double endAngle) const
{
	...
	for (int i = 0; i <= segments; ++i)
	{
		...
		result.push_back({center.x + radius * std::cos(angle), center.y + radius * std::sin(angle)});
	}

	return result;
}

std::vector<Vec2d> SketchRenderer::tessellateEllipse(const Vec2d& center, double majorRadius, double minorRadius,
													 double rotation) const
{
	...
	double circumference = std::numbers::pi * (a + b) * (1.0 + (3.0 * h) / (10.0 + std::sqrt(4.0 - 3.0 * h)));
...
void SketchRenderer::buildVBOs()
{
	...
	Vec3d snapColor = snapActive ? snapColorForType(snapIndicator_.type) : renderStyle.colors.constraintIcon;
	...
	impl_->buildVBOs(entityRenderData_, regionRenderData_, renderStyle, regionDisplayMode_, suppressRegionFill_,
					 entitySelections_, selectedRegions_, hoverRegion_, hoverEntity_, viewport_, pixelScale_,
					 visibleConstraints, ghostConstraints_, snapActive, snapType, snapPos, snapSize, snapColor,
					 snapGuideOrigin, snapHasGuide, anchorIndicators_, activeGuides_, dragGuides_);
	vboDirty_ = false;
}
...
bool SketchRenderer::shouldRenderConstraintMarker(const ConstraintRenderData& data) const
{
	...
	if (hasEntitySelection)
	{
		...
		{
			referencesContext = std::find(data.referencedEntities.begin(), data.referencedEntities.end(),
		...
		if (!referencesContext)
		{
			for (const auto& referenced : data.referencedEntities)
			{
				...
				if (it->second == SelectionState::Selected || it->second == SelectionState::Dragging ||
	...
	const bool midpointType = data.type == ConstraintType::Midpoint || data.type == ConstraintType::Horizontal ||
							  data.type == ConstraintType::Vertical || data.type == ConstraintType::Angle ||
							  data.type == ConstraintType::Parallel || data.type == ConstraintType::Perpendicular ||
...
Vec2d SketchRenderer::calculateConstraintIconPosition(const SketchConstraint* /*constraint*/) const
{
	// Not yet implemented
	return {0, 0};
}

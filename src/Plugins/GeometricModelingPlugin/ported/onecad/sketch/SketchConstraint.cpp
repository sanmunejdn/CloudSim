/// @file SketchConstraint.cpp
/// @brief SketchConstraint 实现
...
SketchConstraint::SketchConstraint() : m_id(generateId()) {}

SketchConstraint::SketchConstraint(const ConstraintID& id) : m_id(id.empty() ? generateId() : id) {}
...
bool SketchConstraint::references(const EntityID& entityId) const
{
	...
	return std::any_of(entities.begin(), entities.end(), [&](const EntityID& id) { return id == entityId; });
}
...
bool SketchConstraint::deserializeBase(const QJsonObject& json, const char* expectedType)
{
	...
	ConstraintID newId = json.contains("id") ? json["id"].toString().toStdString() : generateId();
	m_id = std::move(newId);
	return true;
}
...
DimensionalConstraint::DimensionalConstraint(double value) : SketchConstraint(), m_value(value) {}

DimensionalConstraint::DimensionalConstraint(const ConstraintID& id, double value)
	: SketchConstraint(id), m_value(value)
{
}
...
std::unique_ptr<SketchConstraint> ConstraintFactory::fromJson(const QJsonObject& json)
{
	...
	{
		qWarning() << "ConstraintFactory: failed to deserialize type" << QString::fromStdString(typeName);
		return nullptr;
	}

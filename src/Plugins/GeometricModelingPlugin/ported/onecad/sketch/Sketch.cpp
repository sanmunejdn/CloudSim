...
bool isTangentPairSupported(EntityType first, EntityType second)
{
	...
	const bool firstCircular = first == EntityType::Arc || first == EntityType::Circle;
	const bool secondCircular = second == EntityType::Arc || second == EntityType::Circle;
	return (firstLine && secondCircular) || (secondLine && firstCircular) || (firstCircular && secondCircular);
}
...
EntityID Sketch::addPoint(double x, double y, bool construction)
{
	...
	qCDebug(logSketchEngine) << "addPoint:done"
							 << "id=" << QString::fromStdString(id) << "totalEntities=" << entities_.size();
	return id;
}
...
EntityID Sketch::addLine(EntityID startId, EntityID endId, bool construction)
{
	qCDebug(logSketchEngine) << "addLine"
							 ...
							 << "endId=" << QString::fromStdString(endId) << "construction=" << construction;
...
EntityID Sketch::addArc(EntityID centerId, double radius, double startAngle, double endAngle, bool construction)
{
...
EntityID Sketch::addEllipse(EntityID centerId, double majorRadius, double minorRadius, double rotation,
							bool construction)
{
	auto* centerPoint = getEntityAs<SketchPoint>(centerId);
	if (!centerPoint)
...
ConstraintSupportResult Sketch::validateConstraintSupport(const SketchConstraint& constraint) const
{
	...
	case ConstraintType::Coincident:
	{
		...
		if (!c || !hasType(*this, c->point1(), EntityType::Point) || !hasType(*this, c->point2(), EntityType::Point))
		{
	...
	case ConstraintType::Midpoint:
	{
		...
		if (!c || !hasType(*this, c->pointId(), EntityType::Point) || !hasType(*this, c->lineId(), EntityType::Line))
		{
	...
	case ConstraintType::Parallel:
	{
		...
		if (!c || !hasType(*this, c->line1(), EntityType::Line) || !hasType(*this, c->line2(), EntityType::Line))
		{
	...
	case ConstraintType::Tangent:
	{
		...
		if (!first.has_value() || !second.has_value() || !isTangentPairSupported(*first, *second))
		{
	...
	case ConstraintType::Equal:
	{
		...
		const auto first = entityType(c->entity1());
		const auto second = entityType(c->entity2());
		if (!first.has_value() || !second.has_value() || !isEqualPairSupported(*first, *second))
		{
	...
	case ConstraintType::Distance:
	{
		...
		const auto first = entityType(c->entity1());
		const auto second = entityType(c->entity2());
		if (!first.has_value() || !second.has_value() || !isDistancePairSupported(*first, *second))
		{
	...
	case ConstraintType::Concentric:
	{
		...
		const auto first = entityType(c->entity1());
		const auto second = entityType(c->entity2());
		if (!first.has_value() || !second.has_value() || !isCircularCurve(*first) || !isCircularCurve(*second))
		{
...
ConstraintID Sketch::addConstraint(std::unique_ptr<SketchConstraint> constraint)
{
	...
	for (const auto& entityId : constraint->referencedEntities())
	{
		...
		{
			qCWarning(logSketchEngine) << "addConstraint:invalid-reference" << QString::fromStdString(entityId);
			return {};
		}
		if (referenced->isReferenceLocked() && constraint->type() != ConstraintType::Fixed)
		{
			qCWarning(logSketchEngine) << "addConstraint:reference-locked" << QString::fromStdString(entityId)
	...
	qCDebug(logSketchEngine) << "addConstraint:done"
							 << "id=" << QString::fromStdString(id) << "totalConstraints=" << constraints_.size();
	return id;
}
...
{
	if (!getEntityAs<SketchPoint>(point1) || !getEntityAs<SketchPoint>(point2) || !getEntityAs<SketchLine>(axisLine))
	{
		return {};
	}
...
ConstraintID Sketch::addFixed(EntityID entityId)
{
	...
	return addConstraint(std::make_unique<constraints::FixedConstraint>(entityId, x, y));
}
...
bool Sketch::hasFixedConstraint(EntityID pointId) const
{
	...
	{
		if (constraint && constraint->type() == ConstraintType::Fixed && constraint->references(pointId))
		{
			return true;
		}
	}
...
void Sketch::translateSketchRegion(const std::string& regionId, double dx, double dy)
{
	...
	for (auto& constraint : constraints_)
	{
		...
		auto* fc = dynamic_cast<constraints::FixedConstraint*>(constraint.get());
		if (fc && pointIds.find(fc->pointId()) != pointIds.end() && !isEntityReferenceLocked(fc->pointId()))
		{
			fc->translate(dx, dy);
		}
	}
...
ConstraintID Sketch::addPointOnCurve(EntityID pointId, EntityID curveId, CurvePosition position)
{
	...
	if (curveType != EntityType::Arc && curveType != EntityType::Circle && curveType != EntityType::Line)
	{
		qCWarning(logSketchEngine) << "addPointOnCurve:unsupported-curve" << QString::fromStdString(curveId)
	...
	return addConstraint(std::make_unique<constraints::PointOnCurveConstraint>(pointId, curveId, finalPosition));
}
...
std::vector<Vec2d> Sketch::getPointFreeDirections(EntityID pointId) const
{
	...
	for (const auto& constraint : constraints_)
	{
		...
		if (constraint->type() == ConstraintType::Fixed || constraint->type() == ConstraintType::Coincident)
		{
			return {}; // 0 free DOF
		}
	}
...
SolveResult Sketch::solve()
{
	...
	if (const auto unsupported = firstUnsupportedConstraint(*this))
	{
		...
		qCWarning(logSketchEngine) << "solve:unsupported-constraint" << QString::fromStdString(*unsupported);
		return result;
	}
...
SolveResult Sketch::solveWithGroupDrag(const std::unordered_map<EntityID, Vec2d>& targetPositions)
{
	SolveResult result;
...
SolveResult Sketch::solveWithDrag(EntityID draggedPoint, const Vec2d& targetPos)
{
	...
	if (const auto unsupported = firstUnsupportedConstraint(*this))
	{
		result.success = false;
		result.errorMessage = *unsupported;
		qCWarning(logSketchEngine) << "solveWithDrag:unsupported-constraint" << QString::fromStdString(*unsupported);
		if (isDraggingPoint_)
		{
	...
	const std::unordered_set<EntityID>& pointIdsToFix = isDraggingPoint_ ? activeDragFixedPoints_ : kNoFixedPoints;
...
std::unique_ptr<Sketch> Sketch::fromJson(const std::string& json)
{
	...
	if (root.contains("hostFace") && root["hostFace"].isObject())
	{
		...
		{
			if (!hostFace.contains("projectedBoundaryVersion") || !hostFace["projectedBoundaryVersion"].isDouble())
			{
				return nullptr;
			}
...
EntityID Sketch::findNearest(const Vec2d& pos, double tolerance, std::optional<EntityType> filter) const
{
	EntityID bestId;
...
void Sketch::invalidateSolver()
{
	...
	qCDebug(logSketchEngine) << "invalidateSolver"
							 << "entityCount=" << entities_.size() << "constraintCount=" << constraints_.size();
}

void Sketch::rebuildSolver()
{
	qCDebug(logSketchEngine) << "rebuildSolver:start"
							 << "entities=" << entities_.size() << "constraints=" << constraints_.size();

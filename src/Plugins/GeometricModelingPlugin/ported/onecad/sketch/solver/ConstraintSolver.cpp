...
bool lineEndpoints(const std::unordered_map<EntityID, SketchPoint*>& pointsById, const SketchLine* line,
				   SketchPoint*& start, SketchPoint*& end)
{
	if (!line)
	{
		return false;
...
bool circleCenter(const std::unordered_map<EntityID, SketchPoint*>& pointsById, const SketchCircle* circle,
				  SketchPoint*& center)
{
	if (!circle)
	{
...
bool arcCenter(const std::unordered_map<EntityID, SketchPoint*>& pointsById, const SketchArc* arc, SketchPoint*& center)
{
	if (!arc)
	{
		return false;
...
ConstraintSolver::ConstraintSolver() : config_(), gcsSystem_(std::make_unique<GCS::System>())
{
	configureSystem();
}

ConstraintSolver::ConstraintSolver(const SolverConfig& config)
	: config_(config), gcsSystem_(std::make_unique<GCS::System>())
{
	configureSystem();
}
...
void ConstraintSolver::clear()
{
	qCDebug(logConstraintSolver) << "clear"
								 << "entities(points,lines,arcs,circles)=" << pointsById_.size() << linesById_.size()
								 << arcsById_.size() << circlesById_.size() << "constraints=" << constraints_.size();
...
bool ConstraintSolver::addConstraint(SketchConstraint* constraint)
{
	...
	qCDebug(logConstraintSolver) << "addConstraint:done"
								 << "constraintId=" << QString::fromStdString(constraint->id()) << "tagId=" << tagId
...
void ConstraintSolver::removeConstraint(ConstraintID id)
{
	...
	constraints_.erase(std::remove_if(constraints_.begin(), constraints_.end(),
									  [&](const SketchConstraint* c) { return c && c->id() == id; }),
					   constraints_.end());
}
...
SolverResult ConstraintSolver::solve()
{
	...
	qCDebug(logConstraintSolver) << "solve:start"
								 << "points=" << pointsById_.size() << "lines=" << linesById_.size()
								 << "arcs=" << arcsById_.size() << "circles=" << circlesById_.size()
								 << "constraints=" << constraints_.size() << "parameters=" << parameters_.size()
											 ...
											 const std::unordered_set<EntityID>& pointIdsToFix)
{
	qCDebug(logConstraintSolver) << "solveWithDrag:start"
								 << "pointId=" << QString::fromStdString(pointId) << "target=" << targetPos.x
								 << targetPos.y << "fixedPoints=" << pointIdsToFix.size();
	...
	if (substeps > 1)
	{
		qCInfo(logConstraintSolver) << "solveWithDrag:substep-triggered"
									<< "pointId=" << QString::fromStdString(pointId) << "delta=" << deltaMagnitude
									<< "steps=" << substeps;
	}
...
SolverResult ConstraintSolver::solveWithGroupDrag(const std::unordered_map<EntityID, Vec2d>& targetPositions)
{
	...
	if (substeps > 1)
	{
		qCInfo(logConstraintSolver) << "solveWithGroupDrag:substep-triggered"
									<< "targetPoints=" << targetPositions.size() << "maxDelta=" << maxDeltaMagnitude
									<< "steps=" << substeps;
	}
...
SolverResult ConstraintSolver::solveWithDragSingleStep(EntityID pointId, const Vec2d& targetPos,
													   const std::unordered_set<EntityID>& pointIdsToFix)
{
...
SolverResult ConstraintSolver::solveWithGroupDragSingleStep(const std::unordered_map<EntityID, Vec2d>& targetPositions)
{
	if (!gcsSystem_)
	{
		SolverResult result;
	...
	for (const auto& [pointId, targetPos] : targetPositions)
	{
		...
		tempTargets.emplace_back(makePoint(pointIt->second), std::make_pair(targetPos.x, targetPos.y));
	}
...
bool ConstraintSolver::translateConstraint(SketchConstraint* constraint, int tagId)
{
	...
	if (auto* parallel = dynamic_cast<ParallelConstraint*>(constraint))
	{
		...
		if (!lineEndpoints(pointsById_, line1, l1s, l1e) || !lineEndpoints(pointsById_, line2, l2s, l2e))
		{
			return false;
		}
	...
	if (auto* horizontalDistance = dynamic_cast<HorizontalDistanceConstraint*>(constraint))
	{
		...
		gcsSystem_->addConstraintDifference(coordPtr(p1, 1), coordPtr(p2, 1), horizontalDistance->valuePtr(), tagId,
											true);
		return true;
	}
	...
	if (auto* verticalDistance = dynamic_cast<VerticalDistanceConstraint*>(constraint))
	{
		...
		gcsSystem_->addConstraintDifference(coordPtr(p1, 2), coordPtr(p2, 2), verticalDistance->valuePtr(), tagId,
											true);
		return true;
	}
	...
	if (auto* tangent = dynamic_cast<TangentConstraint*>(constraint))
	{
		...
		if (circle1 && circle2)
		{
			...
			if (!circleCenter(pointsById_, circle1, c1) || !circleCenter(pointsById_, circle2, c2))
			{
				return false;
			}
		...
		if (arc1 && arc2)
		{
			SketchPoint* c1 = nullptr;
			SketchPoint* c2 = nullptr;
			if (!arcCenter(pointsById_, arc1, c1) || !arcCenter(pointsById_, arc2, c2))
			{
				return false;
			}
		...
		if (circle1 && arc2)
		{
			SketchPoint* c1 = nullptr;
			SketchPoint* c2 = nullptr;
			if (!circleCenter(pointsById_, circle1, c1) || !arcCenter(pointsById_, arc2, c2))
			{
				return false;
			}
		...
		if (arc1 && circle2)
		{
			SketchPoint* c1 = nullptr;
			SketchPoint* c2 = nullptr;
			if (!arcCenter(pointsById_, arc1, c1) || !circleCenter(pointsById_, circle2, c2))
			{
				return false;
			}

/// @file IntersectionManager.cpp
/// @brief IntersectionManager 实现
...
IntersectionResult IntersectionManager::processIntersections(EntityID newEntityId, Sketch& sketch,
															 const SnapManager& snapManager)
{
	...
	{
		auto intersections = findIntersections(newEntity, existingEntity, sketch, snapManager);
	...
	for (auto& [entityId, points] : entityIntersections)
	{
		...
		if (entity->type() == EntityType::Line)
		{
			...
			std::sort(splits.begin(), splits.end(), [](const SplitPoint& a, const SplitPoint& b) { return a.t < b.t; });
		...
		else if (entity->type() == EntityType::Arc)
		{
			...
			for (const auto& pt : points)
			{
				...
				if (arc->containsAngle(angle))
				{
					...
					if (angleDiff(angle, startAngle) > 0.01 && angleDiff(angle, endAngle) > 0.01)
					{
			...
			std::sort(splits.begin(), splits.end(),
					  [](const SplitAngle& a, const SplitAngle& b) { return a.angle < b.angle; });
...
std::vector<Vec2d> IntersectionManager::findIntersections(const SketchEntity* e1, const SketchEntity* e2,
														  const Sketch& sketch, const SnapManager& snapManager) const
{
	// Delegate to SnapManager's intersection finding logic
	return snapManager.findEntityIntersections(e1, e2, sketch);
}

EntityID IntersectionManager::findExistingPointAt(const Vec2d& pos, const Sketch& sketch, double tolerance) const
{
...
std::vector<Vec2d> IntersectionManager::mergeNearbyPoints(const std::vector<Vec2d>& points, double tolerance) const
{
	if (points.empty())
	{
		return {};

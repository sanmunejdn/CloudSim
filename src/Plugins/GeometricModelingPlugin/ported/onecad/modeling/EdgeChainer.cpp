/// @file EdgeChainer.cpp
/// @brief EdgeChainer 实现
...
struct EdgeHash
{
	size_t operator()(const TopoDS_Edge& e) const { return std::hash<void*>()(e.TShape().get()); }
};

struct EdgeEqual
{
	bool operator()(const TopoDS_Edge& a, const TopoDS_Edge& b) const { return a.IsSame(b); }
};

} // namespace

EdgeChainer::ChainResult EdgeChainer::buildChain(const TopoDS_Edge& seed, const TopoDS_Shape& body,
	...
	while (!frontier.empty())
	{
		...
		for (const auto& vertex : {v1, v2})
		{
			...
			for (TopTools_ListIteratorOfListOfShape it(adjacentList); it.More(); it.Next())
			{
				...
				if (neighbor.IsNull() || neighbor.IsSame(current))
					continue;
...
bool EdgeChainer::areTangentContinuous(const TopoDS_Edge& e1, const TopoDS_Edge& e2, double tolerance)
{
...
std::vector<TopoDS_Edge> EdgeChainer::adjacentEdges(const TopoDS_Edge& edge, const TopoDS_Shape& body)
{
	std::vector<TopoDS_Edge> result;
	if (edge.IsNull() || body.IsNull())
		return result;
...
gp_Dir EdgeChainer::tangentAtVertex(const TopoDS_Edge& edge, const TopoDS_Vertex& vertex)
{
...
TopoDS_Vertex EdgeChainer::findSharedVertex(const TopoDS_Edge& e1, const TopoDS_Edge& e2)
{
	if (e1.IsNull() || e2.IsNull())
	...
	if (!v1a.IsNull() && (v1a.IsSame(v2a) || v1a.IsSame(v2b)))
		return v1a;
	if (!v1b.IsNull() && (v1b.IsSame(v2a) || v1b.IsSame(v2b)))
		return v1b;

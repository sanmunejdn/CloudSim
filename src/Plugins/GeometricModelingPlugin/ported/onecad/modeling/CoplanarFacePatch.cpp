/// @file CoplanarFacePatch.cpp
/// @brief CoplanarFacePatch 实现
...
bool isCoplanarWithSeed(const TopoDS_Face& candidate, const gp_Pnt& seedOrigin, const gp_Dir& seedNormal,
						double normalDotTolerance, double planeDistanceTolerance, bool& needsReverseOut)
{
...
std::vector<TopoDS_Face> CoplanarFacePatch::collectConnectedFaces(const TopoDS_Shape& body, const TopoDS_Face& seedFace,
		...
		while (!frontier.empty())
		{
			...
			for (TopExp_Explorer edgeExplorer(current, TopAbs_EDGE); edgeExplorer.More(); edgeExplorer.Next())
			{
				...
				for (TopTools_ListIteratorOfListOfShape it(adjacentFaces); it.More(); it.Next())
				{
					...
					if (!isCoplanarWithSeed(neighbor, seedPlane.Location(), seedNormal, normalDotTolerance,

/// @file SketchDragGestureCommand.cpp
/// @brief SketchDragGestureCommand 实现
...
{
SketchDragGestureCommand::SketchDragGestureCommand(Document* document, std::string sketchId, std::string label)
	: document_(document), sketchId_(std::move(sketchId)), label_(std::move(label))
{
}
...
bool SketchDragGestureCommand::finalizeGesture()
{
	...
	changed_ = (beforeJson_ != afterJson_) || (beforeName_ != afterName_) || (beforeVisible_ != afterVisible_);
	finalized_ = true;
	return true;
}
...
bool SketchDragGestureCommand::snapshotSketchState(std::string* outJson, std::string* outName, bool* outVisible) const
{
...
bool SketchDragGestureCommand::restoreSketchState(const std::string& json, const std::string& name, bool visible)
{

...
{
	qCDebug(logDependencyGraph) << "rebuildFromOperations:start"
	...
	qCDebug(logDependencyGraph) << "rebuildFromOperations:done"
								<< "nodeCount=" << nodes_.size() << "forwardEdgeCount=" << forwardEdges_.size()
...
void DependencyGraph::removeOperation(const std::string& opId)
{
	...
	creationOrder_.erase(std::remove(creationOrder_.begin(), creationOrder_.end(), opId), creationOrder_.end());
...
void DependencyGraph::collectDownstream(const std::string& opId, std::unordered_set<std::string>& visited,
...
void DependencyGraph::collectUpstream(const std::string& opId, std::unordered_set<std::string>& visited,
									  std::vector<std::string>& result) const
{

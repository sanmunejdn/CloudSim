...
std::string inputSourceSummary(const OperationInput& input)
{
	...
	if (std::holds_alternative<FaceRef>(input))
	{
		...
		return "FaceRef(" + ref.bodyId + "," + ref.faceId + ",patch=" + std::to_string(ref.patchFaceIds.size()) + ")";
	}
...
bool checkedShapeResult(const TopoDS_Shape& shape, const std::string& context, std::string& errorOut)
{
	if (shape.IsNull())
	{
...
TopoDS_Shape checkedBooleanResult(const TopoDS_Shape& target, const TopoDS_Shape& tool, BooleanMode mode,
								  const std::string& context, std::string& errorOut)
{
...
std::optional<core::modeling::FacePatchSelection>
resolveFacePatchSelection(const FaceRef& faceRef, const TopoDS_Shape& body,
						  const kernel::elementmap::ElementMap& elementMap, std::string& errorOut,
						  bool& usedLegacyFallbackOut)
{
	...
	auto legacyPatch = core::modeling::FacePatchResolver::resolveFromSeedFaceId(body, elementMap, faceRef.faceId);
	if (!legacyPatch)
	{
...
RegenerationEngine::RegenerationEngine(Document* doc) : doc_(doc), graph_()
{
	qCDebug(logRegen) << "RegenerationEngine:ctor"
					  << "hasDocument=" << (doc_ != nullptr);
	if (doc_)
	{
...
RegenResult RegenerationEngine::regenerateToAppliedCount(std::size_t appliedCount)
{
	...
	for (const auto& bodyId : replayOutputBodyIds)
	{
		...
		qCDebug(logRegen) << "regenerateAll:pre-reset-body"
						  << "bodyId=" << QString::fromStdString(bodyId) << "bodyExisted=" << bodyExisted
	...
	for (const auto& opId : order)
	{
		...
		{
			qCDebug(logRegen) << "regenerateAll:skip-suppressed" << QString::fromStdString(opId);
		...
		if (!opRecord)
		{
			qCWarning(logRegen) << "regenerateAll:missing-operation-record" << QString::fromStdString(opId);
			result.skippedOps.push_back(opId);
			continue;
		}
		...
		if (upstreamFailed)
		{
			qCWarning(logRegen) << "regenerateAll:skip-upstream-failed" << QString::fromStdString(opId);
		...
		if (success)
		{
			qCDebug(logRegen) << "regenerateAll:operation-succeeded" << QString::fromStdString(opId);
		...
		else
		{
			qCWarning(logRegen) << "regenerateAll:operation-failed"
								...
								<< "resultBodies="
								<< QString::fromStdString(joinResultBodyIds(opRecord->resultBodyIds));
	...
	qCInfo(logRegen) << "regenerateAll:done"
					 << "status=" << static_cast<int>(result.status) << "succeeded=" << result.succeededOps.size()
					 << "failed=" << result.failedOps.size() << "skipped=" << result.skippedOps.size();
...
std::optional<TopoDS_Shape> RegenerationEngine::resolveEdge(const std::string& edgeId) const
{
	...
	TopoDS_Shape recovered = doc_->elementMap().resolveWithFallback(kernel::elementmap::ElementId{edgeId},
																	kRematchRejectScore, rematchScore);
	if (!recovered.IsNull())
	{
		qCWarning(logRegen) << "resolveEdge:reference-remapped"
							<< "edgeId=" << QString::fromStdString(edgeId) << "score=" << rematchScore;
		return recovered;
	}
	return std::nullopt;
}
...
std::optional<TopoDS_Shape> RegenerationEngine::resolveFace(const std::string& faceId) const
{
	...
	double rematchScore = 0.0;
	TopoDS_Shape recovered = doc_->elementMap().resolveWithFallback(kernel::elementmap::ElementId{faceId},
																	kRematchRejectScore, rematchScore);
	if (!recovered.IsNull())
	{
		qCWarning(logRegen) << "resolveFace:reference-remapped"
							<< "faceId=" << QString::fromStdString(faceId) << "score=" << rematchScore;
		return recovered;
	}
	return std::nullopt;
}
...
bool RegenerationEngine::executeOperation(const OperationRecord& op, std::string& errorOut)
{
	qCDebug(logRegen) << "executeOperation:start"
					  << "opId=" << QString::fromStdString(op.opId) << "type=" << static_cast<int>(op.type)
...
TopoDS_Shape RegenerationEngine::buildExtrude(const OperationRecord& op, std::string& errorOut)
{
	try
	{
		...
		const bool distanceDriven = !params.twoDirections && (params.extrudeMode == ExtrudeMode::Blind ||
															  params.extrudeMode == ExtrudeMode::Symmetric);
						...
						if (!box.IsVoid())
						{
							...
							for (int corner = 0; corner < 8; ++corner)
							{
								const gp_Pnt p((corner & 1) ? xmax : xmin, (corner & 2) ? ymax : ymin,
											   (corner & 4) ? zmax : zmin);
								maxProj = std::max(maxProj, gp_Vec(prismOrigin, p).Dot(gp_Vec(refDir)));
							}
		...
		{
			if (params.extrudeMode == ExtrudeMode::Symmetric || params.extrudeMode2 == ExtrudeMode::Symmetric)
			{
				errorOut = "Symmetric is not valid with two directions";
				return {};
			}
			const gp_Dir dir2 = direction.Reversed();
			auto d1 = effectiveDistance(params.extrudeMode, params.distance, params.targetFaceId, direction, errorOut);
			if (!d1)
			{
				return {};
			}
			auto d2 = effectiveDistance(params.extrudeMode2, params.distance2, params.targetFaceId2, dir2, errorOut);
			if (!d2)
			{
				return {};
			}
		...
		if (params.booleanMode != BooleanMode::NewBody)
		{
			...
			{
				errorOut =
					"Boolean target body is required for mode " + std::string(booleanModeName(params.booleanMode));
...
TopoDS_Shape RegenerationEngine::buildRevolve(const OperationRecord& op, std::string& errorOut)
{
	...
	else if (std::holds_alternative<FaceRef>(op.input))
	{
		...
		auto patch =
			resolveFacePatchSelection(faceRef, *bodyOpt, doc_->elementMap(), errorOut, usedLegacyPatchFallback);
		if (!patch)
		{
			return {};
		}
		...
		qCDebug(logRegen) << "buildRevolve:coplanar-patch"
						  ...
						  << "patchFaces=" << profileFaces.size() << "patchFaceIds=" << patch->memberFaceIds.size()
	...
	catch (const Standard_Failure& failure)
	{
		errorOut = "Revolve failed: " +
				   std::string(failure.GetMessageString() ? failure.GetMessageString() : "OCCT construction failure");
		return {};
	}
	catch (const std::exception& ex)
	{
	...
	if (params.booleanMode != BooleanMode::NewBody)
	{
		...
		if (targetBodyId.empty())
		{
			errorOut = "Boolean target body is required for mode " + std::string(booleanModeName(params.booleanMode));
...
TopoDS_Shape RegenerationEngine::buildShell(const OperationRecord& op, std::string& errorOut)
{
	...
	try
	{
		...
		thickSolid.MakeThickSolidByJoin(targetShape, facesToRemove, -params.thickness, 1e-3, BRepOffset_Skin, false,
										false, GeomAbs_Arc, false);
...
TopoDS_Shape RegenerationEngine::buildBoolean(const OperationRecord& op, std::string& errorOut)
{
	...
	{
		errorOut = "Boolean target references a body created by a later operation: " + params.targetBodyId;
		return {};
	}
	if (!graph_.producesBefore(params.toolBodyId, op.opId))
	{
		errorOut = "Boolean tool references a body created by a later operation: " + params.toolBodyId;
		return {};
	}
...
std::string RegenerationEngine::resolveLegacySketchHostBodyId(const OperationInput& input) const
{
	...
	if (!sketch)
	{
		qCWarning(logRegen) << "resolveLegacySketchHostBodyId:sketch-not-found" << QString::fromStdString(ref.sketchId);
		return {};
	}
	...
	{
		qCDebug(logRegen) << "resolveLegacySketchHostBodyId:no-host-face" << QString::fromStdString(ref.sketchId);
		return {};
	}
...
bool RegenerationEngine::applyBodyResult(const std::string& bodyId, const TopoDS_Shape& shape, const std::string& opId,
										 std::string& errorOut)
{
	if (!doc_)
	{
...
TopoDS_Shape RegenerationEngine::buildLoft(const OperationRecord& op, std::string& errorOut)
{
	try
	{
		...
		if (p.profileSketchIds.size() < 2 || p.profileSketchIds.size() != p.profileRegionIds.size())
		{

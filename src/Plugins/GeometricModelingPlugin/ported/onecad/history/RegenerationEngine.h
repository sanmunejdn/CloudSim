 ...
 */
#ifndef GEOMETRICMODELINGPLUGIN_REGENERATIONENGINE_H
#define GEOMETRICMODELINGPLUGIN_REGENERATIONENGINE_H
...
private:
     ...
     */
	std::optional<TopoDS_Face> buildFaceFromSketchRegion(const std::string& sketchId, const std::string& regionId,
														 std::string& errorOut);
     ...
     */
	std::string resolveBooleanTargetBodyId(const OperationRecord& op, const std::string& explicitTargetBodyId) const;

	/**
     * @brief Add or update a body in the document.
     */
	bool applyBodyResult(const std::string& bodyId, const TopoDS_Shape& shape, const std::string& opId,
						 std::string& errorOut);
...
#endif // GEOMETRICMODELINGPLUGIN_REGENERATIONENGINE_H

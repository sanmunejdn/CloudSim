...
PlcCommWidget::PlcCommWidget(QWidget* parent)
    : QWidget(parent)
    , controller_(new PlcCommController(this))
    , pollTimer_(new QTimer(this))
{
    ...
    connect(pollIntervalSpin_, QOverload<int>::of(&QSpinBox::valueChanged), this, [this](int ms) {
...
{
    const bool modbus = protocolCombo_->currentData().toInt()
        == static_cast<int>(PlcProtocol::ModbusTcp);
    ...
    tagNameEdit_->setPlaceholderText(modbus
        ? i18n(QStringLiteral("40001 or hr0 (holding reg)"),
            QStringLiteral("40001 或 hr0（保持寄存器）"))
...
void PlcCommWidget::onWriteClicked()
{
    ...
    if (!parseValueText(valueEdit_->text(), &bytes)) {
        QMessageBox::warning(this, i18n(QStringLiteral("PLC"), QStringLiteral("PLC")),
            i18n(QStringLiteral("Invalid value for selected format"),
                QStringLiteral("当前格式下的数值无效")));
        return;
    }
...
void PlcCommWidget::onConnectedChanged(bool connected)
{
    ...
    appendLog(connected
        ? i18n(QStringLiteral("Status: connected"), QStringLiteral("连接状态：已连接"))
...
void PlcCommWidget::updateValueEditPlaceholder()
{
    ...
    case ValueFormat::Hex:
        valueEdit_->setPlaceholderText(
            i18n(QStringLiteral("e.g. 01 02 FF"), QStringLiteral("如 01 02 FF")));
        break;
    case ValueFormat::DecimalBytes:
        valueEdit_->setPlaceholderText(
            i18n(QStringLiteral("e.g. 1 255 0"), QStringLiteral("如 1 255 0")));
        break;
    case ValueFormat::BinaryBytes:
        valueEdit_->setPlaceholderText(
    ...
    case ValueFormat::UInt16Le:
        valueEdit_->setPlaceholderText(
            i18n(QStringLiteral("e.g. 1234"), QStringLiteral("如 1234")));
        break;
    case ValueFormat::Int32Le:
        valueEdit_->setPlaceholderText(
            i18n(QStringLiteral("e.g. -1 100000"), QStringLiteral("如 -1 100000")));
        break;
    }
}

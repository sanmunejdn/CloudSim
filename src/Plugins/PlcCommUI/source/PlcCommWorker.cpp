...
PlcCommWorker::PlcCommWorker(QObject* parent)
    : QObject(parent)
    , client_(createPlcCommClient())
{
}
...
void PlcCommWorker::connectPlc(const PlcConnectionConfig& config)
{
    ...
    if (!ok) {
        ...
        emit logMessage(i18n(QStringLiteral("Modbus TCP connected"),
            QStringLiteral("Modbus TCP 已连通")));
    } else {
        emit logMessage(i18n(
            QStringLiteral("EIP settings saved; session starts when a tag is added"),
...
void PlcCommWorker::addTag(const PlcTagSpec& spec)
{
    ...
    emit logMessage(i18n(QStringLiteral("Added tag %1 -> #%2").arg(
            QString::fromStdString(spec.name)).arg(handle),
...
void PlcCommWorker::removeTag(int handle)
{
    ...
    emit logMessage(i18n(QStringLiteral("Removed tag #%1").arg(handle),
        QStringLiteral("移除标签 #%1").arg(handle)));
}
...
void PlcCommWorker::readTag(int handle)
{
    ...
    QByteArray bytes(reinterpret_cast<const char*>(value.data.data()),

...
ParseResult tryParseUserText(const QString& textIn)
{
	...
	if (t.isEmpty())
	{
		r.errorMessage = QStringLiteral("请输入描述，例如：生成长方体，长100mm，宽50mm，高100mm");
		return r;
	}
	if (!hasCreateVerb(t))
	{
	...
	case Detected::Box:
	{
		...
		if (L < 0 && nums.size() >= 3) { L = nums[0]; W = nums[1]; H = nums[2]; }
		if (L <= 0 || W <= 0 || H <= 0)
		{
			r.errorMessage = QStringLiteral("长方体需要长、宽、高（mm），例如：长100 宽50 高100");
			return r;
		}
		cmd["dimensions_mm"] = { { "length", L }, { "width", W }, { "height", H } };
		cmd["name"] = QStringLiteral("Box_%1x%2x%3").arg(L).arg(W).arg(H).toStdString();
		break;
	}
	case Detected::Cylinder:
	{
		...
		if (t.contains(QStringLiteral("直径")) && nums.size() >= 1)
			R = nums[0] * 0.5;
		if (R < 0 && nums.size() >= 2) { R = nums[0]; H = nums[1]; }
		if (R <= 0 || H <= 0)
		{
			r.errorMessage = QStringLiteral("圆柱需要半径（或直径）和高度，例如：半径30mm 高80mm");
			return r;
		}
		cmd["dimensions_mm"] = { { "radius", R }, { "height", H } };
		cmd["name"] = QString("Cylinder_R%1_H%2").arg(R).arg(H).toStdString();
		break;
	}
	case Detected::Cone:
	{
		...
		if (R <= 0 || H <= 0)
		{
			r.errorMessage = QStringLiteral("圆锥需要底面半径和高度。");
			return r;
		}
		cmd["dimensions_mm"] = { { "radius", R }, { "height", H } };
		cmd["name"] = QString("Cone_R%1_H%2").arg(R).arg(H).toStdString();
		break;
	}
	case Detected::Sphere:
	{
		...
		double R = findLabeled(t, { QStringLiteral("半径") }, nums, 0);
		if (t.contains(QStringLiteral("直径")))
		{
			if (nums.size() >= 1)
			...
			cmd["dimensions_mm"] = { { "radius", R } };
		else if (nums.size() >= 1)
			cmd["dimensions_mm"] = { { "radius", nums[0] } };
		else
			R = -1;
		if (!cmd.contains("dimensions_mm"))
		{
			r.errorMessage = QStringLiteral("球体需要半径或直径。");
			return r;
		}

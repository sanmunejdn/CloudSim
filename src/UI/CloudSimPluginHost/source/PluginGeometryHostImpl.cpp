	...
	std::string* errMsg = nullptr)
{
	...
	const osg::Vec3d pw(
		static_cast<double>(worldMm.x()),
		static_cast<double>(worldMm.y()),
...
void runMeshJob(
	...
	std::function<bool(MeshWorkResult&)> work,
	...
	PluginGeometryFinishedFn onFinished)
{
	...
	host->enqueueJob(
		...
		[host, page, result, options, onFinished = std::move(onFinished)](
...
void runIntersectJob(
	PluginHostContext* host,
	IPluginDocument* doc,
	const QString& jobTitle,
	std::function<bool(IntersectWorkResult&)> work,
	PluginGeometryFinishedFn onFinished)
{
	if (!host || !onFinished)
	{
...
PluginGeometryHostImpl::PluginGeometryHostImpl(PluginHostContext* hostContext)
	: m_host(hostContext)
{
}

void PluginGeometryHostImpl::discretizeStepToMesh(
	IPluginDocument* doc,
	...
	PluginGeometryFinishedFn onFinished)
{
	...
	runMeshJob(
		m_host,
		doc,
		QStringLiteral("STEP discretize"),
		[stepPathUtf8, geoParams](MeshWorkResult& out) {
			return geometry_backend_ops::discretizeStepToMesh(
				stepPathUtf8, geoParams, out.soup, out.report, &out.error);
		},
		options,
		std::move(onFinished));
}

void PluginGeometryHostImpl::discretizeBackendToMesh(
	IPluginDocument* doc,
	const std::string& stepPathUtf8,
	const PluginMeshDiscretizeParams& params,
	const PluginMeshCreateOptions& options,
...
void PluginGeometryHostImpl::discretizeBackendFaceToMesh(
	IPluginDocument* doc,
	...
	PluginGeometryFinishedFn onFinished)
{
	...
	runMeshJob(
		m_host,
		doc,
		QStringLiteral("Face discretize"),
		[faceRef, geoParams](MeshWorkResult& out) {
			return geometry_backend_ops::discretizeStepFaceToMesh(
				faceRef.stepPathUtf8, faceRef.faceIndex, geoParams, out.soup, out.report, &out.error);
		},
		options,
		std::move(onFinished));
}

void PluginGeometryHostImpl::discretizeWireToTubeMesh(
	IPluginDocument* doc,
...
void PluginGeometryHostImpl::discretizeWireToRibbonMesh(
	IPluginDocument* doc,
	const std::vector<float>& polylineXyz,
	const PluginMeshDiscretizeParams& params,
	const PluginMeshCreateOptions& options,
...
void PluginGeometryHostImpl::discretizePolylineToMesh(
	IPluginDocument* doc,
	const std::vector<float>& polylineXyz,
	const PluginMeshDiscretizeParams& params,
	const PluginMeshCreateOptions& options,
	PluginGeometryFinishedFn onFinished)
{
	...
	runMeshJob(
		m_host,
		doc,
		QStringLiteral("Polyline mesh"),
		[polylineXyz, geoParams](MeshWorkResult& out) {
			const bool ok = geometry_backend_ops::discretizePolylineToMesh(
				polylineXyz, geoParams, out.soup, &out.error);
			if (ok)
			{
...
void PluginGeometryHostImpl::discretizeBackendEdgesToPolylines(
	IPluginDocument* doc,
	const std::string& stepPathUtf8,
	const PluginMeshDiscretizeParams& params,
	PluginGeometryFinishedFn onFinished)
{
	...
	runIntersectJob(
		m_host,
		doc,
		QStringLiteral("Edge polylines"),
		[stepPathUtf8, geoParams](IntersectWorkResult& out) {
			return geometry_backend_ops::discretizeStepEdgesToPolylines(
				stepPathUtf8, geoParams.tessellate, out.polylines, &out.error);
		},
		std::move(onFinished));
}

void PluginGeometryHostImpl::intersectEdges(
	IPluginDocument* doc,
	...
	PluginGeometryFinishedFn onFinished)
{
	...
	runIntersectJob(
		m_host,
		doc,
		QStringLiteral("Edge-edge intersect"),
		[path, edge1, edge2, geoParams](IntersectWorkResult& out) {
			return geometry_backend_ops::intersectStepEdges(
				path, edge1.edgeIndex, edge2.edgeIndex, geoParams, out.intersection, &out.error);
		},
		std::move(onFinished));
}

void PluginGeometryHostImpl::intersectEdgeFace(
	IPluginDocument* doc,
	...
	PluginGeometryFinishedFn onFinished)
{
	...
	runIntersectJob(
		m_host,
		doc,
		QStringLiteral("Edge-face intersect"),
		[path, edgeRef, faceRef, geoParams](IntersectWorkResult& out) {
			return geometry_backend_ops::intersectStepEdgeFace(
				path, edgeRef.edgeIndex, faceRef.faceIndex, geoParams, out.intersection, &out.error);
		},
		std::move(onFinished));
}

void PluginGeometryHostImpl::intersectFaces(
	IPluginDocument* doc,
	...
	PluginGeometryFinishedFn onFinished)
{
	...
	runIntersectJob(
		m_host,
		doc,
		QStringLiteral("Face-face intersect"),
		[path, face1, face2, geoParams](IntersectWorkResult& out) {
			return geometry_backend_ops::intersectStepFaces(
				path, face1.faceIndex, face2.faceIndex, geoParams, out.intersection, &out.error);
		},
		std::move(onFinished));
}

void PluginGeometryHostImpl::intersectBackends(
	IPluginDocument* doc,
	...
	PluginGeometryFinishedFn onFinished)
{
	...
	runIntersectJob(
		m_host,
		doc,
		QStringLiteral("Shape intersect"),
		[targetStepPathUtf8, toolStepPathUtf8, geoParams](IntersectWorkResult& out) {
			return geometry_backend_ops::intersectStepFiles(
				targetStepPathUtf8, toolStepPathUtf8, geoParams, out.intersection, &out.error);
		},
		std::move(onFinished));
}

void PluginGeometryHostImpl::brepBooleanToMesh(
	IPluginDocument* doc,
	const std::string& targetStepPathUtf8,
	const std::string& toolStepPathUtf8,
	...
	PluginGeometryFinishedFn onFinished)
{
	...
	runMeshJob(
		m_host,
		doc,
		QStringLiteral("B-rep boolean"),
		[targetStepPathUtf8, toolStepPathUtf8, geoOp, geoMesh](MeshWorkResult& out) {
			const bool ok = geometry_backend_ops::brepBooleanStepFilesToMesh(
				targetStepPathUtf8, toolStepPathUtf8, geoOp, geoMesh, out.soup, &out.error);
			if (ok)
			{
				geometry_backend_ops::fillMeshReport(out.soup, out.report);
			}
...
void PluginGeometryHostImpl::fuseWiresToPolyline(
	IPluginDocument* doc,
	const std::string& stepPathUtf8,
...
void PluginGeometryHostImpl::sewFacesToMesh(
	IPluginDocument* doc,
	const std::string& stepPathUtf8,
	...
	PluginGeometryFinishedFn onFinished)
{
	...
	runMeshJob(
		m_host,
		doc,
		QStringLiteral("Sew faces"),
		[stepPathUtf8, faceIndices, geoParams](MeshWorkResult& out) {
			const bool ok = geometry_backend_ops::sewStepFacesToMesh(
				stepPathUtf8, faceIndices, 1e-3, geoParams, out.soup, &out.error);
			if (ok)
			{
				geometry_backend_ops::fillMeshReport(out.soup, out.report);
			}
...
bool PluginGeometryHostImpl::listComputableBackends(
	IPluginDocument* doc,
...
void PluginGeometryHostImpl::pickStepElementFromViewport(
	IPluginDocument* doc,
	...
	PluginGeometryElementPickedFn onFinished)
{
	...
	state->conn = QObject::connect(osg, &OsgWidget::meshPickCommitted, m_host, [=](PickResult pick, int pickKindInt) {
	...
	QTimer::singleShot(30000, m_host, [complete]() {
		complete(false, QStringLiteral("Pick timeout"), {});
	});
}

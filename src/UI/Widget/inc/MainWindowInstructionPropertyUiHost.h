...
public:
	...
	void appendPropertyBrowserRow(const QString& propertyKey, const QString& displayLabel, const QString& value,
		...
		const QStringList* enumDisplayNames = nullptr, const QString& toolTip = QString()) override;
	...
	RobotInstruction::FeasibleMotionAxisConfigurationOptions feasibleMotionAxisConfigurationOptionsForInstruction(
		const std::shared_ptr<RobotInstruction::Base>& instruction, QVector<double>* outSeedJointRad = nullptr) override;

	cloudsim::core::FeasibleMotionAxisOptionsDto cachedFeasibleMotionAxisOptionsDto() const override;

	void applySuggestedAxisPresetFromSeedIfNeeded(const std::shared_ptr<RobotInstruction::Base>& instruction,

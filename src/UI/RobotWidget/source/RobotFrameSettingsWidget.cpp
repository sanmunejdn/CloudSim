...
void RobotFrameSettingsWidget::setCoordinateFrames(const RobotCoordinate::RobotCoordinateFrameSet& frames)
{
	m_blockSignals = true;
	m_frames = frames;
	RobotCoordinate::ensureUniqueToolFrameIds(m_frames);
...
RobotCoordinate::RobotCoordinateFrameSet RobotFrameSettingsWidget::coordinateFrames()
{
	if (m_framesDebounceTimer && m_framesDebounceTimer->isActive())
	{
		m_framesDebounceTimer->stop();
	}
	saveToolFieldsToSelection();
...
void RobotFrameSettingsWidget::onAddToolFrame()
{
	...
	tf.id = RobotCoordinate::allocateUniqueToolFrameId(m_frames);

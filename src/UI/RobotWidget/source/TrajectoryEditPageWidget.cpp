...
void TrajectoryEditPageWidget::applyPipelineOps(std::vector<RobotInstruction::TrajectoryOpDescriptor> ops)
{
	if (!m_pipeline)
	{
		return;
	}
	m_pipeline->setOps(std::move(ops));
	syncSessionPipeline();

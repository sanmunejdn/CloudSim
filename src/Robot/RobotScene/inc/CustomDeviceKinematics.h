...
/// 合成 W_eff = W0 * Π T_i(q_i)
ROBOT_SCENE_API void composeWorldFromBase(const double w0ColumnMajor[16], const CustomDeviceAxisConfigSet& axes,
										  const std::vector<double>& qValues, double outColumnMajor[16]);

ROBOT_SCENE_API void unbakeBaseFromWorld(const double wEffColumnMajor[16], const CustomDeviceAxisConfigSet& axes,
										 const std::vector<double>& qValues, double outW0ColumnMajor[16]);

/// 漂移则 unbake W0；写 q 与 worldMatrix；经 poseSink 推 OSG

...
void resampleUnifiedTrajectory(RobotInstruction::UnifiedTrajectory& traj, const double stepMm)
{
	...
	for (int s = 0; s < n; ++s)
	{
		...
		const double local =
			(seg < segLen.size() && segLen[seg] > 1e-12) ? (t - acc) / segLen[seg] : 0.0;
...
void offsetAlongNormalUnified(RobotInstruction::UnifiedTrajectory& traj, const double offsetMm)
{
	for (RobotInstruction::UnifiedTrajectoryPoint& tp : traj.points)
	{
		...
		const RobotInstruction::Vec3 nz{
...
void smoothPoseUnified(RobotInstruction::UnifiedTrajectory& traj)
{
	...
	{
		smoothed[i].poseMm.x = (traj.points[i - 1U].poseMm.x + traj.points[i].poseMm.x
			+ traj.points[i + 1U].poseMm.x)
			/ 3.0;
		smoothed[i].poseMm.y = (traj.points[i - 1U].poseMm.y + traj.points[i].poseMm.y
			+ traj.points[i + 1U].poseMm.y)
			/ 3.0;
		smoothed[i].poseMm.z = (traj.points[i - 1U].poseMm.z + traj.points[i].poseMm.z
			+ traj.points[i + 1U].poseMm.z)
			/ 3.0;
	}
...
void weaveUnified(
	...
	const double amplitudeMm,
	const double periodMm)
{
	double acc = 0.0;
	for (std::size_t i = 0; i < traj.points.size(); ++i)
	{
		...
		const double phase =
			(periodMm > 1e-6) ? (acc / periodMm) * 2.0 * kPi : 0.0;
	...
	const double stepMm)
{
	...
	traj.points.erase(
		traj.points.begin() + static_cast<std::ptrdiff_t>(insertAt),
...
{
	forEachScopedPoint(traj, scope, program, [offsetMm](RobotInstruction::UnifiedTrajectoryPoint& tp) {
		const double yaw = tp.eulerDeg.z * kPi / 180.0;
		const double pitch = tp.eulerDeg.y * kPi / 180.0;
		const RobotInstruction::Vec3 nz{
			std::sin(pitch) * std::cos(yaw),
...
void offsetLateralUnifiedInScope(
	...
	const RobotInstruction::OpScope& scope,
	const RobotInstruction::RobotProgram* program,
	const double lateralMm)
{
	forEachScopedPoint(traj, scope, program, [lateralMm](RobotInstruction::UnifiedTrajectoryPoint& tp) {
		const double yaw = tp.eulerDeg.z * kPi / 180.0;
		const RobotInstruction::Vec3 lateral{-std::sin(yaw), std::cos(yaw), 0.0};
		tp.poseMm.x += lateral.x * lateralMm;
		tp.poseMm.y += lateral.y * lateralMm;
...
void assignBlendUnifiedInScope(
	...
	const RobotInstruction::OpScope& scope,
	const RobotInstruction::RobotProgram* program,
	const double blendRadiusMm)
{
	forEachScopedPoint(traj, scope, program, [blendRadiusMm](RobotInstruction::UnifiedTrajectoryPoint& tp) {
		tp.blendRadiusMm = blendRadiusMm;
	});
}
...
{
	forEachScopedPoint(traj, scope, program, [speedMmPerSec](RobotInstruction::UnifiedTrajectoryPoint& tp) {
		tp.speedMmPerSec = speedMmPerSec;
	});
}
...
void reachabilityFilterUnifiedInScope(
	...
	const RobotInstruction::RobotProgram* program)
{
	forEachScopedPoint(traj, scope, program, [](RobotInstruction::UnifiedTrajectoryPoint& tp) {
		tp.reachable = tp.poseMm.z > -5000.0;
	});
}

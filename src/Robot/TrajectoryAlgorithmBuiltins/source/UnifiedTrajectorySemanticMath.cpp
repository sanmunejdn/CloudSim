...
RobotInstruction::TrajectoryOpDescriptor interpolateDescriptor(
...
Eigen::Vector3d tangentDirection(
	...
	const std::size_t index)
{
	...
	const std::size_t next =
		index + 1 >= traj.points.size() ? traj.points.size() - 1 : index + 1;
	Eigen::Vector3d dir(
...
Eigen::Vector3d normalizeOrFallback(
...
Eigen::Vector3d directionByMode(
	...
	const double customX,
	const double customY,
	const double customZ)
{
	switch (mode)
	{
	...
	case RobotInstruction::ApproachDirectionMode::Custom:
		return normalizeOrFallback(
			Eigen::Vector3d(customX, customY, customZ),
...
Eigen::Vector3d resolveApproachDirection(
	const RobotInstruction::UnifiedTrajectory& traj,
	const std::size_t anchorIndex,
	const RobotInstruction::ApproachDirectionMode mode,
	const RobotInstruction::TransformReferenceFrame frame,
	const double customX,
	const double customY,
	const double customZ)
{
...
bool segmentIndexInRange(
	const int segmentIndex1Based,
	const RobotInstruction::SegmentSelectMode mode,
	const int from,
	const int to)
{
...
void appendApproachAnchors(
	const std::vector<std::size_t>& indices,
	const RobotInstruction::InsertMode insertMode,
	const RobotInstruction::SegmentSelectMode segmentMode,
	const int segmentFrom,
	const int segmentTo,
	std::vector<std::size_t>& outAnchors)
{
	if (indices.empty())
	{
		return;
...
void pointFromRigid(
	const engine::RigidTransform& tf,
	RobotInstruction::UnifiedTrajectoryPoint& point)
{
	double px = 0.0;
	double py = 0.0;
	double pz = 0.0;
	...
	const RobotInstruction::RobotProgram* program)
{
	...
	appendApproachAnchors(
		indices,
		params.insertMode,
		params.segmentSelectMode,
		params.segmentFrom,
		params.segmentTo,
		anchors);
	if (anchors.empty())
	...
	for (auto it = anchors.rbegin(); it != anchors.rend(); ++it)
	{
		...
		const Eigen::Vector3d dir = resolveApproachDirection(
			traj,
			anchor,
			params.directionMode,
			params.directionFrame,
			params.customDirectionX,
			params.customDirectionY,
			params.customDirectionZ);
	...
	const RobotInstruction::RobotProgram* program)
{
	...
	for (auto it = anchors.rbegin(); it != anchors.rend(); ++it)
	{
		...
		traj.points.insert(
			traj.points.begin() + static_cast<std::ptrdiff_t>(anchor + 1U),
			point);
	}
}

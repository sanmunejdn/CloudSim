...
ReorderOp::makeDefaultDescriptor(const RobotInstruction::OpScope& defaultScope) const
{
...
{
	return {
		messageParamField("reorder.fixedPoseHint",
...
std::string ReorderOp::formatSummary(const RobotInstruction::TrajectoryOpDescriptor& op, const bool chinese) const
{
	(void)op;
	return chinese ? "固定姿态 | 首点姿态" : "Fixed Orientation | Use first waypoint";
}

bool ReorderOp::processPath(const RobotInstruction::TrajectoryOpDescriptor& op,

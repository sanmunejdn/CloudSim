...
TrajectoryOpParamField boolParamField(
...
RobotInstruction::TrajectoryOpDescriptor NonRigidRegistrationOp::makeDefaultDescriptor(
...
std::vector<TrajectoryOpParamField> NonRigidRegistrationOp::paramFields() const
{
	return {
		messageParamField(
			"nrr.sourceBackendId",
			"Source geometry backend (point cloud or mesh).",
			"源几何 backend（点云或 mesh）。",
			0),
		messageParamField(
			"nrr.targetBackendId",
			"Target geometry backend (point cloud or mesh).",
			"目标几何 backend（点云或 mesh）。",
			1),
		doubleParamField(
			"nrr.maxBindDistanceMm",
			"Max Bind Distance",
			"绑定最大距离",
			"mm",
			0.1,
...
bool NonRigidRegistrationOp::validate(
	const RobotInstruction::TrajectoryOpDescriptor& op,
	std::string* errMsg) const
{
	const RobotInstruction::NonRigidRegistrationParams nrr =
		parseNonRigidRegistrationParams(op.params);
...
std::string NonRigidRegistrationOp::formatSummary(
	const RobotInstruction::TrajectoryOpDescriptor& op,
	const bool chinese) const
{
	...
	const RobotInstruction::NonRigidRegistrationParams nrr =
		parseNonRigidRegistrationParams(op.params);
	return std::string("NonRigidRegistration ")
		+ nrr.sourceBackendId + " -> " + nrr.targetBackendId;
}

bool NonRigidRegistrationOp::processPath(
	const RobotInstruction::TrajectoryOpDescriptor& op,
	...
	std::string* errMsg) const
{
	...
	return ctx.nonRigidTrajectoryWarp->warp(
		traj,
		parseNonRigidRegistrationParams(op.params),
		op.scope,
		ctx.program,

...
| 3 | 拉伸 startOffset·双向深度、圆周阵列、成角基准面 | **已交付**（，Host ABI **1.48.0**） |
| 4 | TopoNaming 自研渐进 | **下期独立专题**（见 `docs/拓扑命名/`） |
| — | JSON / Python 脚本建模 | **已交付**（；一期无 ABI bump） |
...
见 `docs/ARCHIVE_ZIP_LOCATION.txt`：Ribbon 导出/导入 history、运行 compose；进程内 `cloudsim_geom` + 控制台。不做 headless。

## 5. 包 A 交付摘要（已完成）

见 `docs/ARCHIVE_ZIP_LOCATION.txt`：A0 命名参数、A1 椭圆 GCS、A2 Convert 保弧、A3 样条双模式；Debug+Release 已编过。

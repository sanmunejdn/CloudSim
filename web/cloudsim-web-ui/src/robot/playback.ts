    ...
    poll = window.setInterval(() => {
      void (async () => {
        ...
        try {
          ...
          if (st.ok && !st.running) end(true, st.abortSummary);
        } catch {
          /* 轮询兜底 */
        }
      })();
...
export function buildPlanBody(
  ...
  step: Instruction,
  jointRadCsvFallback = "",
  seedPolicy: "FromInstruction" | "FromCurrentPose" = "FromInstruction",
) {
...
export async function planStepFrames(
  ...
  const r = await planInstruction(buildPlanBody(rootId, step, jointCsv, seedPolicy));
